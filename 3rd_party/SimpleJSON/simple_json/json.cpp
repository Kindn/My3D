...
namespace sjson
{
    ...
    std::string JsonReader::strAhead(size_t n) const
    {
        return src_str.substr(curr_pos, std::min<size_t>(n, 
                              src_str_len - curr_pos));
    }

    std::string JsonReader::getStr(size_t n)
    {
    ...
    bool JsonNode::set_array(size_t _size)
    {
    ...
    JsonNode& JsonNode::array_get(size_t _index)
    {
        if (value_type != JSON_VALUE_TYPE_ARRAY)
        {
            std::stringstream msg;
    ...
    const JsonNode& JsonNode::array_get(size_t _index) const
    {
        if (value_type != JSON_VALUE_TYPE_ARRAY)
        {
            std::stringstream msg;
    ...
    void JsonNode::array_insert(JsonNode_P _node, size_t _index)
    {
        if (value_type != JSON_VALUE_TYPE_ARRAY)
        {
            std::stringstream msg;
    ...
    void JsonNode::array_erase(size_t _index)
    {
        if (value_type != JSON_VALUE_TYPE_ARRAY)
        {
            std::stringstream msg;
    ...
    bool JsonNode::obj_has_item(const std::string& _key) const
    {
        if (value_type != JSON_VALUE_TYPE_OBJECT)
        {
            std::stringstream msg;
    ...
    const JsonNode& JsonNode::obj_get_item(const std::string& _key) const
    {
        if (value_type != JSON_VALUE_TYPE_OBJECT)
        {
            std::stringstream msg;
    ...
    const JsonNode& JsonNode::operator [] (const std::string& _key) const
    {
        if (value_type != JSON_VALUE_TYPE_OBJECT)
            throw JsonError("Value type mismatched!", 
                            0, 
        ...
        {
            throw JsonError("Item with key [" + _key + "] does not exist", 
                            0, 
                            0, 
                            false);
        }
    ...
    JsonNode& JsonNode::operator [] (size_t _array_ind)
    {
        if (value_type == JSON_VALUE_TYPE_NULL)
            setValueType(JSON_VALUE_TYPE_ARRAY);
        if (value_type != JSON_VALUE_TYPE_ARRAY)
    ...
    const JsonNode& JsonNode::operator [] (size_t _array_ind) const
    {
        if (value_type != JSON_VALUE_TYPE_ARRAY)
            throw JsonError("Value type mismatched!", 
                            0, 
    ...
    std::vector<JsonNode_P> JsonNode::as_vector() const
    {
