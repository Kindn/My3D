...
double rad2Deg(const double &val_rad) {
    return val_rad * 180.0 / M_PI; 
}

double deg2Rad(const double &val_deg) {
    return val_deg * M_PI / 180.0; 
}
...
size_t computeCombinations(const size_t &n, 
    ...
    if (k > n || n == 0) {
        ...
        if (combinations) {
            ...
            std::iota(combinations->at(0).begin(), combinations->at(0).end(), static_cast<size_t>(0)); 
        }
        return 1; 
    } 
...
double computeGaussian(const double &x, const double &sigma, const double &mean) {
...
double computeGaussian(const Eigen::VectorXd &x, 
    ...
    const double a = 1.0 / std::sqrt(std::pow(2.0 * M_PI, num_dims) * covariance.determinant()); 
