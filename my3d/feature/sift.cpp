...
SIFTKeyPoint::SIFTKeyPoint(const Eigen::Vector2d &_point,
                           const double &_response, const double &_edge_score,
                           const size_t &_octave_idx,
                           const size_t &_valid_level_idx, const double &_scale)
    : point{_point}, response{_response}, edge_score{_edge_score},
      octave_idx{_octave_idx}, valid_level_idx{_valid_level_idx}, scale{
                                                                      _scale} {}

SIFTDescriptor::SIFTDescriptor(const size_t &_key_point_idx,
                               const Eigen::Vector2d &_point,
                               const double &_orientation, const double &_scale,
                               const Eigen::Matrix<double, 8, 16> &_histograms)
    : key_point_idx{_key_point_idx}, point{_point},
      orientation{_orientation}, scale{_scale}, histograms{_histograms} {}

SIFTDetector::Octave::Octave(const size_t _octave_idx,
                             const base::Image &_base_image,
                             const size_t &_num_valid_dog_levels,
                             const double &_scale_factor, const double &_sigma,
                             const double &_image_scale_factor)
    : octave_idx{_octave_idx}, gaussian_space{_base_image,
                                              _num_valid_dog_levels + 3,
                                              _scale_factor, _sigma},
      image_scale_factor{_image_scale_factor} {
  // std::cout << "octave: " << _num_valid_dog_levels << std::endl;
  dog_space.build(gaussian_space);
}

SIFTDetector::SIFTDetector(const Config &config) : config_{config} {
  this->config(config);
  std::cout << "num_valid_dog_levels: " << config_.num_valid_levels_per_octave
            << std::endl;
}
...
void SIFTDetector::config(const Config &config) {
  ...
  if (config_.num_valid_levels_per_octave == 0) {
    ...
    if (config_.verbose) {
      std::cout << "[WARNING] SIFTDetector: "
                ...
                << "Set it to default value: " << kDefautNumValidLevelsPerOctave
                << std::endl;
    }
  }
  if (config_.num_octaves == 0) {
    ...
    if (config_.verbose) {
      std::cout << "[WARNING] SIFTDetector: "
                ...
                << "Set it to default value: " << kDefaultNumOctaves
                << std::endl;
    }
  }
  if (config_.octave_idx_base_image > 0 && config_.num_octaves > 0 &&
    ...
    if (config_.verbose) {
      std::cout << "[WARNING] SIFTDetector: "
                ...
                << "Set it to default value: " << kDefaultOctaveIdxBaseImage
                << std::endl;
    }
  }
  if (config_.min_abs_resp < 0.0) {
    ...
    if (config_.verbose) {
      std::cout << "[WARNING] SIFTDetector: "
                ...
                << "Set it to default value: " << kDefaultMinAbsDoGResponse
                << std::endl;
    }
  }
  if (config_.edge_score_thresh < 0.0) {
    ...
    if (config_.verbose) {
      std::cout << "[WARNING] SIFTDetector: "
                ...
                << "Set it to default value: " << kDefaultEdgeScoreThresh
                << std::endl;
    }
  }
  if (config_.base_sigma <= std::numeric_limits<double>::epsilon()) {
    ...
    if (config_.verbose) {
      std::cout << "[WARNING] SIFTDetector: "
                << "Invalid base sigma. "
                << "Set it to default value: " << kDefaultBaseSigma
                << std::endl;
    }
  }
}
...
void SIFTDetector::detect(SIFTKeyPointList &key_points) {
  key_points.clear();
  const double edge_score_thresh =
      std::pow(config_.edge_score_thresh + 1.0, 2.0) /
      config_.edge_score_thresh;
  // For all the octaves
  for (const auto &octave : octaves_) {
    // For each valid DoG level
    for (size_t valid_level_idx = 0;
         valid_level_idx < config_.num_valid_levels_per_octave;
         ++valid_level_idx) {
      // Detect extrema
      std::vector<size_t> xs;
      std::vector<size_t> ys;
      std::vector<double> vals;
      octave.dog_space.detectExtrema(valid_level_idx, xs, ys, vals,
                                     config_.min_abs_resp);
      if (config_.verbose) {
        std::cout << "\t==> [INFO] SIFTDetector: " << xs.size()
                  << " extrema detected in octave " << octave.octave_idx
      ...
      for (size_t i = 0; i < xs.size(); ++i) {
        ...
        double edge_score;
        Eigen::Vector3d pos;
        bool is_valid = octave.dog_space.computeSubPixelExtremumPosition(
            xs[i], ys[i], valid_level_idx, pos, &response, &edge_score);
        if (is_valid &&
            std::abs(response) >=
                config_.min_abs_resp /
                    static_cast<double>(config_.num_valid_levels_per_octave) &&
            ...
            pos.y() / octave.image_scale_factor <= image_.rows() - 1 && true) {
          // std::cout << edge_score << "   ";
          const double scale = octave.gaussian_space.getSigma(pos.z()) /
                               octave.image_scale_factor;
          key_points.emplace_back(
              pos.head<2>() / octave.image_scale_factor, response, edge_score,
      ...
      if (config_.verbose) {
        // std::cout << std::endl;
        std::cout << "\t==> [INFO] SIFTDetector: " << num_kpts
                  << " key-points detected in octave " << octave.octave_idx
                  << ", level: " << valid_level_idx + 1 << std::endl;
      }
    }
  }
...
void SIFTDetector::createOctaves() {
  ...
  if (octave_idx_base_image < 0) {
    if (config_.max_width <= image_.cols() ||
      ...
      const long int sw = static_cast<long int>(
          std::log2(static_cast<double>(config_.max_width) /
                    static_cast<double>(image_.cols())));
      const long int sh = static_cast<long int>(
          std::log2(static_cast<double>(config_.max_height) /
  ...
  long int num_octaves =
      (config_.num_octaves <= 0)
          ? std::max<long int>(
                1L, std::log2l(std::min(image_.rows(), image_.cols())) - 3L +
  ...
  const double scale_factor = std::pow(
      2.0, 1.0 / static_cast<double>(config_.num_valid_levels_per_octave));
  ...
  for (long int oct_idx = 0; oct_idx < num_octaves; ++oct_idx) {
    ...
    octaves_.emplace_back(static_cast<size_t>(oct_idx), curr_image,
                          config_.num_valid_levels_per_octave, scale_factor,
                          config_.base_sigma, curr_image_scale_factor);
    if (config_.verbose) {
      std::cout << "\t ==> [INFO] SIFTDetector: "
                << "Added octave " << oct_idx << ". " << std::endl
                << "\t\timage size: " << curr_image.cols() << "x"
                << curr_image.rows() << std::endl
                << "\t\timage scaling factor: " << curr_image_scale_factor
                << std::endl
                << "\t\tnum Gaussian levels: "
                << octaves_.back().gaussian_space.getNumLevels() << std::endl
                << "\t\tnum DoG levels: "
                << octaves_.back().dog_space.getNumLevels() << std::endl
                << "\t\toctave idx of base image: " << octave_idx_base_image
                << std::endl
...
#define COMPUTE_SECONDARY_ORIENTATIONS 1
  ...
  // const base::Image &image = octave.gaussian_space[key_point.valid_level_idx
  // + 1]; const double relative_scale =
  // octave.dog_space.getSigma(key_point.valid_level_idx + 1);
  ...
  // if (center_x - half_patch_size < 0 || center_x + half_patch_size >=
  // static_cast<int>(width) ||
  //     center_y - half_patch_size < 0 || center_y + half_patch_size >=
  //     static_cast<int>(height)) { std::cout << "[WARNING] SIFTDetector: "
  //               << "Patch exceeds image limit. "
  //               << "No orientation generated for key-point [" <<
  //               key_point.point.transpose() << "] "
  ...
  for (int dx = -half_patch_size; dx <= half_patch_size; ++dx) {
    ...
    for (int dy = -half_patch_size; dy <= half_patch_size; ++dy) {
      ...
      // image.getGrandient(static_cast<size_t>(x),
      //                                                     static_cast<size_t>(y));
      const Eigen::Vector2d gradient = octave.gaussian_space.computeGradient(
      ...
      // util::clamp<int>(static_cast<int>(kNumBins * angle / (2.0 * M_PI)), 0,
      // kNumBins - 1);
      const double weight =
          std::exp(-0.5 * distance2 / std::pow(1.5 * relative_scale, 2.0));
      // util::computeGaussian(std::sqrt(distance2), 1.5 * relative_scale, 0.0);
      histogram((bin_idx + kNumBins) % kNumBins) +=
          weight * (double(bin_idx) + 1.0 - bin) * gradient.norm();
      histogram((bin_idx + 1) % kNumBins) +=
          weight * (bin - double(bin_idx)) * gradient.norm();
    }
  }
  ...
  if (orientations.empty() && config_.verbose) {
    std::cout << "[WARNING] SIFTDetector: "
              << "No orientation generated for key-point ["
              << key_point.point.transpose() << "] "
              << "in octave " << key_point.octave_idx << " level "
              << key_point.valid_level_idx << std::endl;
  }

  // std::cout << "Generated " << orientations.size() << " orientations. " <<
  // std::endl;
}
...
#define INTERP_GRAD 0
...
#if IMG_TO_DES
  // if (center_x - half_patch_size < 1 || center_x + half_patch_size >=
  // static_cast<int>(width) - 1 ||
  //     center_y - half_patch_size < 1 || center_y + half_patch_size >=
  //     static_cast<int>(height) - 1) {
  //     // if (config_.verbose) {
  //     //     std::cout << "[WARNING] SIFTDetector: "
  //     //           << "No descriptor generated for key-point [" <<
  //     position.transpose() << "] "
  //     //           << "in octave " << key_point.octave_idx << "(" << width <<
  //     "x" << height << ") "
  //     //           << " level " << key_point.valid_level_idx << " "
  //     //           << "half patch size: " << half_patch_size << " "
  //     //           << "(" << center_x - half_patch_size << " " << center_x +
  //     half_patch_size << " " << width - 1 << ") "
  //     //           << "(" << center_y - half_patch_size << " " << center_y +
  //     half_patch_size << " " << height - 1 << ") " << std::endl;
  //     // }
  //     return false;
  // }
#endif
  ...
  for (int dx_des = -half_patch_size; dx_des <= half_patch_size; ++dx_des) {
    for (int dy_des = -half_patch_size; dy_des <= half_patch_size; ++dy_des) {
      ...
      const double dx_acc =
          static_cast<double>(dx_img + center_x) - position.x();
      const double dy_acc =
          static_cast<double>(dy_img + center_y) - position.y();
      ...
      const double x_img =
          static_cast<double>(center_x) + static_cast<double>(dx_img);
      const double y_img =
          static_cast<double>(center_y) + static_cast<double>(dy_img);
#else
      ...
      if (x_img1 < 1 || x_img1 > static_cast<int>(width) - 2 || x_img2 < 1 ||
          x_img2 > static_cast<int>(width) - 2 || y_img1 < 1 ||
          y_img1 > static_cast<int>(height) - 2 || y_img2 < 1 ||
          y_img2 > static_cast<int>(height) - 2) {
        continue;
      }
      const Eigen::Vector2d g11 = octave.gaussian_space.computeGradient(
          x_img1, y_img1, key_point.valid_level_idx + 1);
      const Eigen::Vector2d g12 = octave.gaussian_space.computeGradient(
          x_img1, y_img2, key_point.valid_level_idx + 1);
      const Eigen::Vector2d g21 = octave.gaussian_space.computeGradient(
          x_img2, y_img1, key_point.valid_level_idx + 1);
      const Eigen::Vector2d g22 = octave.gaussian_space.computeGradient(
          x_img2, y_img2, key_point.valid_level_idx + 1);
      const Eigen::Vector2d gradient = util::computeBilinearInterpolation(
          g11, g12, g21, g22, x_img1, x_img2, y_img1, y_img2, x_img, y_img);
#else
      const int x_img1 = static_cast<int>(std::floor(x_img + 0.5));
      const int y_img1 = static_cast<int>(std::floor(y_img + 0.5));
      if (x_img1 < 1 || x_img1 > static_cast<int>(width) - 2 || y_img1 < 1 ||
          y_img1 > static_cast<int>(height) - 2) {
        continue;
      }
      const Eigen::Vector2d gradient = octave.gaussian_space.computeGradient(
          x_img1, y_img1, key_point.valid_level_idx + 1);
      ...
      const double bin_r2 = (bin_x - bin_off) * (bin_x - bin_off) +
                            (bin_y - bin_off) * (bin_y - bin_off);
      const double bin_ori = ori_des * size_ori_hist / (2.0 * M_PI) - 0.5;
      // std::cout << bin_x << " " << bin_y << " " << bin_ori << std::endl;

      const int bin_xs[2] = {static_cast<int>(std::floor(bin_x)),
                             static_cast<int>(std::floor(bin_x)) + 1};
      const int bin_ys[2] = {static_cast<int>(std::floor(bin_y)),
                             static_cast<int>(std::floor(bin_y)) + 1};
      int bin_oris[2] = {static_cast<int>(std::floor(bin_ori)),
                         static_cast<int>(std::floor(bin_ori)) + 1};
      // Weights
      const double sigma = 0.5 * static_cast<double>(d);
      const double gaussian_weight = std::exp(-0.5 * bin_r2 / (sigma * sigma));
      // util::computeGaussian(r, sigma, 0.0);// * std::sqrt(2.0 * M_PI) *
      // sigma;
      const double bin_x_ws[2] = {1.0 - (bin_x - bin_xs[0]), bin_x - bin_xs[0]};
      const double bin_y_ws[2] = {1.0 - (bin_y - bin_ys[0]), bin_y - bin_ys[0]};
      const double bin_ori_ws[2] = {1.0 - (bin_ori - bin_oris[0]),
                                    bin_ori - bin_oris[0]};
      ...
      for (int x = 0; x < 2; ++x) {
        for (int y = 0; y < 2; ++y) {
          for (int ori = 0; ori < 2; ++ori) {
            ...
            hists(bin_oris[ori], hist_idx) += gradient.norm() *
                                              gaussian_weight * bin_x_ws[x] *
                                              bin_y_ws[y] * bin_ori_ws[ori];
            // std::cout << gradient.norm() * gaussian_weight * bin_x_ws[x] *
            // bin_y_ws[x] * bin_ori_ws[ori] << std::endl;
          }
        }
      }
    }
...
void drawSIFTKeyPoints(const base::Image &src, base::Image &dst,
                       ...
                       const uint8_t color[3], const double scale,
  ...
  for (const auto &des : descriptors) {
    ...
    const Eigen::Vector2d end(
        start.x() + des.scale * scale * std::cos(des.orientation),
...
// void drawSIFTMatches(const base::Image &image1, const SIFTDescriptorList
// &descriptors1,
//                      const base::Image &image2, const SIFTDescriptorList
//                      &descriptors2, const Matches &matches, base::Image &out)
//                      {
