...
SIFTExtractor::SIFTExtractor(
  ...
  const SIFTExtractor::Config &derived_config =
      dynamic_cast<const SIFTExtractor::Config &>(config);
  assert(derived_config.check());
  SIFTDetector::Config detector_config;
  detector_config.num_valid_levels_per_octave =
      derived_config.num_valid_levels_per_octave;
...
EigenVec<Eigen::VectorXd> SIFTExtractor::descriptorsToVectors(
    const SIFTDescriptorList &descriptors) const {
...
SIFTExtractor::getKeyPointPositions(const SIFTKeyPointList &key_points) const {
...
EigenVec<Eigen::Matrix<uint8_t, 3, 1>>
SIFTExtractor::getKeyPointColors(const base::Image &src,
                                 const SIFTKeyPointList &key_points) const {
  const size_t num_kpts = key_points.size();
