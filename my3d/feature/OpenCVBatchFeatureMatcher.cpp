...
OpenCVBatchFeatureMatcher::OpenCVBatchFeatureMatcher(
    FeatureMatcherBase<cv::Mat> *matcher)
    : BatchFeatureMatcherBase<std::vector<cv::KeyPoint>, cv::Mat>(matcher) {}

OpenCVBatchFeatureMatcher::~OpenCVBatchFeatureMatcher() {}

void OpenCVBatchFeatureMatcher::match(
    const FeatureImageList<std::vector<cv::KeyPoint>, cv::Mat> &feature_images,
  ...
  for (size_t i = 0; i < num_images - 1; ++i) {
    for (size_t j = i + 1; j < num_images; ++j) {
      ...
      std::cout << "[OpenCVBatchFeatureMatcher] Found "
