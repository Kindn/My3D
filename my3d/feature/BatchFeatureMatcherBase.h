...
public: 
    ...
    explicit BatchFeatureMatcherBase(FeatureMatcherBase<DescriptorListType> *matcher): 
    matcher_{matcher} {
    ...
    virtual void match(const FeatureImageList<KeyPointListType, DescriptorListType> &feature_images, 
        ...
        for (size_t i = 0; i < num_images - 1; ++i) {
            for (size_t j = i + 1; j < num_images; ++j) {
                ...
                std::cout << "[BatchFeatureMatcherBase] Found " << matching.matches.size() << 
        ...
        //     matcher_->match(feature_images[i].descriptors, feature_images[i + 1].descriptors, 
        //                     matching.matches);
        //     pair_wise_matching.push_back(matching);
        //     std::cout << "[BatchFeatureMatcherBase] Found " << matching.matches.size() << 
        //                 " matches between " << feature_images[i].image_idx << " and " << feature_images[i + 1].image_idx << std::endl; 
        // }
    }
    
    // virtual bool write(const std::string &path, 
    //                    const std::vector<PairWiseMatchingInfo> &pair_wise_matching) = 0;
