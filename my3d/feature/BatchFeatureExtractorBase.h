...
public:
  ...
  explicit BatchFeatureExtractorBase(
      FeatureExtractorBase<KeyPointListType, DescriptorListType> *extractor)
  ...
  virtual void extract(
    ...
    for (size_t i = 0; i < num_images; ++i) {
      ...
      feature_image.colors =
          extractor_->getKeyPointColors(images[i], key_points);
      ...
      std::cout << "[BatchFeatureExtractorBase] "
                << "Extracted " << key_points.size() << " features from image "
                << images[i].getName() << std::endl;
    }
  }

  // virtual bool write(const std::string &path,
  //                    const FeatureImageList<KeyPointListType,
  //                    DescriptorListType> &feature_images) = 0;
...
public:
  ...
  explicit BatchFeatureExtractorMultiThreadBase(
      FeatureExtractorBase<KeyPointListType, DescriptorListType> *extractor,
      const size_t &min_num_images_per_thread = 1)
      : BatchFeatureExtractorBase<KeyPointListType, DescriptorListType>(
            extractor) {
...
protected:
  virtual void threadExtraction(
      const std::vector<base::Image> &all_images,
    ...
    const std::shared_ptr<
        ...
        extractor = this->extractor_->clone();
    ...
    for (const size_t &i : image_idxs) {
      ...
      feature_image.descriptors = descriptors;
      feature_image.colors =
          this->extractor_->getKeyPointColors(all_images[i], key_points);
      feature_image.points_2D =
          this->extractor_->getKeyPointPositions(key_points);
      feature_image.name = all_images[i].getName();
      if (feature_images != nullptr) {
        // mtx_feature_images_.lock();
        feature_images->push_back(feature_image);
        std::cout << "[BatchFeatureExtractorBase] "
                  << "Extracted " << key_points.size()
                  << " features from image " << all_images[i].getName()
                  << std::endl;
        // mtx_feature_images_.unlock();
      }
    }
...
public:
  ...
  size_t getMinNumImagesPerThread() const { return min_num_images_per_thread_; }
  ...
  virtual size_t computeNumThreads(const size_t &num_images) const {
    ...
    const size_t max_num_threads =
        (num_images + min_num_images_per_thread_ - 1) /
        min_num_images_per_thread_;
    const size_t hardware_concurrency = std::thread::hardware_concurrency();

    return std::min(hardware_concurrency != 0 ? hardware_concurrency : 2,
                    max_num_threads);
  }

  virtual std::vector<std::vector<size_t>>
  assignImages(const size_t &num_images, const size_t &num_threads) const {
    ...
    const size_t num_images_per_thread =
        std::max(num_images / num_threads, min_num_images_per_thread_);
    for (size_t thread_idx = 0; thread_idx < num_threads; ++thread_idx) {
      for (size_t i = 0;
           i < num_images_per_thread && curr_image_idx < num_images; ++i) {
  ...
  virtual void extract(const std::vector<base::Image> &images,
                       FeatureImageList<KeyPointListType, DescriptorListType>
    ...
    EigenVec<FeatureImageList<KeyPointListType, DescriptorListType>>
        temp_feature_images_lists(num_threads);
    ...
    const std::vector<std::vector<size_t>> assignment =
        assignImages(num_images, num_threads);
    std::cout << "[INFO] Creating " << num_threads << " threads... "
              << std::endl;
    for (size_t i = 0; i < num_threads; ++i) {
      if (!assignment[i].empty()) {
        threads.emplace_back(
            &BatchFeatureExtractorMultiThreadBase::threadExtraction, this,
            images, assignment[i], &temp_feature_images_lists[i]);
