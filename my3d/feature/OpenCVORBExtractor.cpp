...
FeatureExtractorBase<std::vector<cv::KeyPoint>, cv::Mat>(config) {
    extractor_ = cv::ORB::create(config.max_feature_num, 
                                 config.pyramid_scale_factor, 
                                 config.pyramid_levels, 
                                 config.edge_threshold, 
                                 config.level_of_the_first_image, 
                                 config.WTA_K, 
                                 config.score_type, 
                                 config.patch_size, 
    ...
    std::cout << "\tpyramid_scale_factor: " << config.pyramid_scale_factor << std::endl; 
    std::cout << "\tpyramid_levels: " << config.pyramid_levels << std::endl; 
    std::cout << "\tedge_threshold: " << config.edge_threshold << std::endl; 
    std::cout << "\tlevel_of_the_first_image: " << config.level_of_the_first_image << std::endl; 
...
EigenVec<Eigen::VectorXd> OpenCVORBExtractor::descriptorsToVectors(const cv::Mat &descriptors) const {
...
EigenVec<Eigen::Vector2d> OpenCVORBExtractor::getKeyPointPositions(const std::vector<cv::KeyPoint> &key_points) const {
...
EigenVec<Eigen::Matrix<uint8_t, 3, 1>> OpenCVORBExtractor::getKeyPointColors(const base::Image &src, const std::vector<cv::KeyPoint> &key_points) const {
    const size_t num_kpts = key_points.size(); 
