...
public: 
    ...
    NNMatcher() = delete; 
    explicit NNMatcher(const typename FeatureMatcherBase<DescriptorListType>::Config &config): 
    ...
    void NNMatch(const DescriptorListType &query_descriptors, 
        ...
        nn->setDistanceFunction([this](const NNElement &elem1, const NNElement &elem2) {
            const double eucl_dist = this->dist_func_(elem1.descriptor, elem2.descriptor); 
            const double angle_dist = std::acos(std::min(0.5 * (2.0 - dist_norm_ * eucl_dist), 1.0)); 
            return angle_dist; 
        }); 
        ...
        size_t num_matches = 0; 
        for (long int q_idx = 0; q_idx < static_cast<int>(query_descriptors.size()); ++q_idx) {
