...
public:
  struct Config {
    ...
    virtual bool check() const {
      return max_feature_num >= 0 && min_image_size > 0 && max_image_size > 0;
    }
  };
...
public:
  ...
  FeatureExtractorBase(
      const FeatureExtractorBase<KeyPointListType, DescriptorListType> &other) {

  }
  // FeatureExtractorBase<KeyPointListType, DescriptorListType> &operator =
  // (const FeatureExtractorBase<KeyPointListType, DescriptorListType> &other) {
  ...
  virtual void extract(const base::Image &src, KeyPointListType &key_points,
                       DescriptorListType &descriptors) = 0;

  virtual EigenVec<Eigen::VectorXd>
  descriptorsToVectors(const DescriptorListType &descriptors) const = 0;

  virtual EigenVec<Eigen::Vector2d>
  getKeyPointPositions(const KeyPointListType &key_points) const = 0;

  virtual EigenVec<Eigen::Matrix<uint8_t, 3, 1>>
  getKeyPointColors(const base::Image &src,
                    const KeyPointListType &key_points) const = 0;
};
