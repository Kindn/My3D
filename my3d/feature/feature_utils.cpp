...
GaussianSpace::GaussianSpace(const base::Image &base_image,
                             ...
                             const double &sigma):
width_{base_image.cols()}, height_{base_image.rows()},
sigma_{sigma},
...
bool GaussianSpace::build(const base::Image &base_image,
    ...
    if (base_image.channels() == 3) {
        ...
        std::cout << __PRETTY_FUNCTION__
                  << "[ERROR] The base image given for Gaussian space building should be a BGR or gray-scale image, "
                  << "but the given image has channel " << base_image.channels() << std::endl;
        return false;
    } else {
    ...
    for (size_t level = 1; level < num_levels; ++level) {
        ...
        const double incre_sigma = std::sqrt(current_sigma * current_sigma - last_sigma * last_sigma);
        ...
        std::cout << "\r[INFO] Building Gaussian space. " << level + 1 << "/" << num_levels;
...
Eigen::Vector2d GaussianSpace::computeGradient(const size_t &x, const size_t &y, const size_t &level_idx) const {
...
DoGSpace::DoGSpace(const base::Image &base_image,
                   ...
                   const double &scale_factor,
                   const double &sigma):
width_{base_image.cols()},
height_{base_image.rows()},
sigma_{sigma},
scale_factor_{scale_factor}
{
...
bool DoGSpace::build(const GaussianSpace &gaussian_space) {
    ...
    for (size_t i = 0; i < levels_.size(); ++i) {
        ...
        assert(gaussian_space[i + 1].rows() == static_cast<Eigen::Index>(height_) &&
               gaussian_space[i + 1].cols() == static_cast<Eigen::Index>(width_)); // &&
            //    gaussian_space[i + 1].channels() == 1);
        levels_[i] = (gaussian_space[i + 1] - gaussian_space[i]);// / 255.0; 
            // base::computeChannelWiseImageDifference(gaussian_space[i + 1],
            //                                         gaussian_space[i])[0] / 255.0;
    }
...
bool DoGSpace::build(const base::Image &base_image,
    ...
    if (!gaussian_space.build(base_image,
                              sigma,
                              scale_factor,
...
bool DoGSpace::isExtremumPoint(const size_t &valid_level_idx,
                               const size_t &x,
                               const size_t &y, 
                               const double &thresh) const {
    assert(width_ >= 3 && height_ >= 3);
    const size_t num_valid_levels = getNumValidDoGLevels();
    if (valid_level_idx >= num_valid_levels ||
        num_valid_levels == 0) {
        return false;
    }
    ...
    bool is_extremum = 
        (value <= -thresh && 
         value < curr_level(y - 1, x - 1) && 
         value < curr_level(y - 1, x) && 
         value < curr_level(y - 1, x + 1) && 
         value < curr_level(y, x - 1) && 
         value < curr_level(y, x + 1) && 
         value < curr_level(y + 1, x - 1) && 
         value < curr_level(y + 1, x) && 
         value < curr_level(y + 1, x + 1) && 
         value < last_level(y - 1, x - 1) && 
         value < last_level(y - 1, x) && 
         value < last_level(y - 1, x + 1) && 
         value < last_level(y, x - 1) && 
         value < last_level(y, x) && 
         value < last_level(y, x + 1) && 
         value < last_level(y + 1, x - 1) && 
         value < last_level(y + 1, x) && 
         value < last_level(y + 1, x + 1) && 
         value < next_level(y - 1, x - 1) && 
         value < next_level(y - 1, x) && 
         value < next_level(y - 1, x + 1) && 
         value < next_level(y, x - 1) && 
         value < next_level(y, x) && 
         value < next_level(y, x + 1) && 
         value < next_level(y + 1, x - 1) && 
         value < next_level(y + 1, x) && 
         value < next_level(y + 1, x + 1)) || 
...
bool DoGSpace::computeSubPixelExtremumPosition(const size_t &x,
                                               const size_t &y,
    ...
    for (size_t i = 0; i < kNumIterations; ++i) {
        x0 += step_x;
        y0 += step_y;
        if (x0 < 1 || x0 >= static_cast<int>(width_) - 1 || 
            y0 < 1 || y0 >= static_cast<int>(height_) - 1) {
            return false; 
        }
        ...
        if (std::abs(hessian.determinant()) <= std::numeric_limits<double>::epsilon()) {
            break;
        }
        ...
        // delta.x() = -inv_hessian(0, 0) * gx - inv_hessian(0, 1) * gy - inv_hessian(0, 2) * gz; 
        // delta.x() = -inv_hessian(1, 0) * gx - inv_hessian(1, 1) * gy - inv_hessian(1, 2) * gz; 
        // delta.x() = -inv_hessian(2, 0) * gx - inv_hessian(2, 1) * gy - inv_hessian(2, 2) * gz; 
    ...
    const double kNumGaussianSpaceLevels = static_cast<double>(getNumLevels()) + 1; 
    if (position.x() < 0 || position.x() > width_ - 1 || 
        position.y() < 0 || position.y() > height_ - 1 || 
        position.z() < 0 || position.z() > kNumGaussianSpaceLevels - 1) {
        return false; 
    }

    if (std::abs(delta.x()) >= 1.5 || 
        std::abs(delta.y()) >= 1.5 || 
        std::abs(delta.z()) >= 1.5) {
        return false; 
    }
...
Eigen::Vector3d DoGSpace::computeGradient(const size_t &x,
                                          const size_t &y,
...
Eigen::Matrix3d DoGSpace::computeHessian(const size_t &x,
                                         const size_t &y,
                                         const size_t &valid_level_idx) const {
    assert(x >= 1 && x < width_ - 1);
    assert(y >= 1 && y < height_ - 1);
    ...
    hessian(0, 0) = hxx; hessian(0, 1) = hxy; hessian(0, 2) = hxs;
    hessian(1, 0) = hxy; hessian(1, 1) = hyy; hessian(1, 2) = hys;
    hessian(2, 0) = hxs; hessian(2, 1) = hys; hessian(2, 2) = hss;
...
bool PairWiseMatchingInfo::serialize(std::ofstream &ofs) const {
    ...
    ofs.write((const char *)(query_image_name.data()), query_image_name.size() + 1); 
    ofs.write((const char *)(train_image_name.data()), train_image_name.size() + 1); 
    if (!ofs.good()) {
        return false; 
    }
...
getInlierMatches(const EigenVec<Eigen::Vector2d> &key_points1, 
    ...
    // Estimate homography
    estimator::LORANSAC<estimator::HomographyEstimator, estimator::HomographyEstimator> estimator_h(ransac_config);  
    estimator::LORANSAC<estimator::HomographyEstimator, estimator::HomographyEstimator>::Report report_h = 
    ...
    estimator::LORANSAC<estimator::FundamentalMatrixSevenPointEstimator, 
                        estimator::FundamentalMatrixEightPointEstimator> estimator_f(ransac_config); 
    estimator::LORANSAC<estimator::FundamentalMatrixSevenPointEstimator, 
                        estimator::FundamentalMatrixEightPointEstimator>::Report report_f = 
        estimator_f.estimate(points_2D1, points_2D2, false); 
    if (report_f.success) {
    ...
    if (report_h.success && report_f.success) {
        ...
        std::cout << "[" << __PRETTY_FUNCTION__ << "]: " 
                  << "Warning: RANSAC estimation failed. Original matches will be returned. " 
                  << std::endl; 
