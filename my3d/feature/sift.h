...
struct SIFTKeyPoint {
  ...
  SIFTKeyPoint(const Eigen::Vector2d &_point, const double &_response,
               const double &_edge_score, const size_t &_octave_idx,
...
struct SIFTDescriptor {
  ...
  SIFTDescriptor(const size_t &_key_point_idx, const Eigen::Vector2d &_point,
  ...
  static double computeDistance(const SIFTDescriptor &desc1,
                                const SIFTDescriptor &desc2) {
    return (desc1.histograms - desc2.histograms).squaredNorm();
    // double sqr_dist = 0.0;
    // for (size_t i = 0; i < 128; ++i) {
    //     sqr_dist += std::pow(desc1.histograms.coeff(i) -
    //     desc2.histograms.coeff(i), 2.0);
    // }
    // return sqr_dist;
  }
};
...
/**
 * @brief SIFT feature detector. Used for key-point extraction and descriptor
 * computation.
 */
class SIFTDetector {
public:
  ...
  struct Octave {
    Octave() {}
    Octave(const size_t _octave_idx, const base::Image &_base_image,
           const size_t &_num_valid_dog_levels, const double &_scale_factor,
           const double &_sigma, const double &_image_scale_factor);
...
protected:
  ...
  bool computeDescriptor(const SIFTKeyPoint &key_point,
                         const double orientation, SIFTDescriptor &descriptor);
...
void drawSIFTKeyPoints(const base::Image &src, base::Image &dst,
                       ...
                       const uint8_t color[3], const double scale = 1.0,
...
//                      &descriptors2, const Matches &matches, base::Image
//                      &out);
