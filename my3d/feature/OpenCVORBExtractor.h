...
*/
class OpenCVORBExtractor : public FeatureExtractorBase<std::vector<cv::KeyPoint>, cv::Mat> {
public: 
    struct Config : public FeatureExtractorBase<std::vector<cv::KeyPoint>, cv::Mat>::Config {
        ...
        WTA_K(kDefaultWTA_K), 
        score_type(kDefaultORBScoreType), 
        patch_size(kDefaultPatchSize), 
        fast_threshold(kDefaultFastThreshold) {
    ...
    FeatureExtractorBase<std::vector<cv::KeyPoint>, cv::Mat>(other) {
        if (other.extractor_ != nullptr) {
            extractor_ = cv::ORB::create(other.extractor_->getMaxFeatures(), 
    ...
    //     FeatureExtractorBase<std::vector<cv::KeyPoint>, cv::Mat>::operator=(other); 
    //     extractor_ = cv::ORB::create(other.extractor_->getMaxFeatures(), 
    //                                  other.extractor_->getScaleFactor(), 
    //                                  other.extractor_->getNLevels(), 
    //                                  other.extractor_->getEdgeThreshold(), 
    ...
    virtual std::shared_ptr<FeatureExtractorBase<std::vector<cv::KeyPoint>, cv::Mat>> clone() const override {
        return std::shared_ptr<FeatureExtractorBase<std::vector<cv::KeyPoint>, cv::Mat>>(new OpenCVORBExtractor(*this)); 
    }

    virtual void extract(const base::Image& src, std::vector<cv::KeyPoint> &key_points, cv::Mat &descriptors) override; 

    virtual EigenVec<Eigen::VectorXd> descriptorsToVectors(const cv::Mat &descriptors) const override;
    
    virtual EigenVec<Eigen::Vector2d> getKeyPointPositions(const std::vector<cv::KeyPoint> &key_points) const override; 

    virtual EigenVec<Eigen::Matrix<uint8_t, 3, 1>> getKeyPointColors(const base::Image &src, const std::vector<cv::KeyPoint> &key_points) const override;  
