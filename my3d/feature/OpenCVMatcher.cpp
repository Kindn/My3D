...
void OpenCVMatcher::match(const cv::Mat &query_descriptors,
                          const cv::Mat &train_descriptors, Matches &matches) {
  ...
  // const auto min_max = std::minmax_element(original_matches.begin(),
  // original_matches.end(),
  //     [](const cv::DMatch &m1, const cv::DMatch &m2) { return m1.distance <
  //     m2.distance; });
  ...
  for (const auto &match : original_matches) {
    ...
    if (match.size() >= 2) {
      ...
      if (match[0].distance < ratio_thresh_ * match[1].distance) {
        ...
        matches.push_back(
            Match(match[0].queryIdx, match[0].trainIdx, match[0].distance));
      }
    }
  }
