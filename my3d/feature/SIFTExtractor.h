...
public: 
    struct Config : 
        public FeatureExtractorBase<SIFTKeyPointList, SIFTDescriptorList>::Config {
        /* Number of valid DoG levels per octave */
        size_t num_valid_levels_per_octave{SIFTDetector::kDefautNumValidLevelsPerOctave}; 
        /* Number of octaves */
        ...
        virtual bool check() const override {
            bool ret = FeatureExtractorBase<SIFTKeyPointList, SIFTDescriptorList>::Config::check(); 
            return ret && 
                   num_valid_levels_per_octave > 0 && 
                   (num_octaves < 0 || (num_octaves > 0 && (long)octave_idx_base_image < num_octaves)) && 
                   min_abs_resp >= 0 && 
                   edge_score_thresh > 0 && 
                   base_sigma > 0; 
        }
...
public: 
    ...
    explicit SIFTExtractor(const FeatureExtractorBase<SIFTKeyPointList, SIFTDescriptorList>::Config &config); 
    ...
    virtual std::shared_ptr<FeatureExtractorBase<SIFTKeyPointList, SIFTDescriptorList>> clone() const override {
        return std::shared_ptr<FeatureExtractorBase<SIFTKeyPointList, SIFTDescriptorList>>(new SIFTExtractor(*this)); 
    }
    ...
    virtual EigenVec<Eigen::VectorXd> descriptorsToVectors(const SIFTDescriptorList &descriptors) const override;
    
    virtual EigenVec<Eigen::Vector2d> getKeyPointPositions(const SIFTKeyPointList &key_points) const override; 

    virtual EigenVec<Eigen::Matrix<uint8_t, 3, 1>> getKeyPointColors(const base::Image &src, const SIFTKeyPointList &key_points) const override;  
