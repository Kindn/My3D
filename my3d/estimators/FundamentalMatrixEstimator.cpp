...
void FundamentalMatrixSevenPointEstimator::estimate(
    const VecXData &x_data, const VecYData &y_data,
  ...
  const double s7 =
      f1(4) * f2(8) + f1(8) * f2(4) - f1(5) * f2(7) - f1(7) * f2(5);
  const double s8 =
      f1(3) * f2(8) + f1(8) * f2(3) - f1(5) * f2(6) - f1(6) * f2(5);
  const double s9 =
      f1(3) * f2(7) + f1(7) * f2(3) - f1(4) * f2(6) - f1(6) * f2(4);

  Eigen::Vector4d coeffs;
  coeffs(3) = f1(0) * s1 - f1(1) * s2 + f1(2) * s3;
  coeffs(2) = f1(0) * s7 + f2(0) * s1 - f1(1) * s8 - f2(1) * s2 + f1(2) * s9 +
              f2(2) * s3;
  coeffs(1) = f1(0) * s4 + f2(0) * s7 - f1(1) * s5 - f2(1) * s8 + f1(2) * s6 +
              f2(2) * s9;
  ...
  for (Eigen::Index i = 0; i < roots_real.size(); ++i) {
    ...
    ModelType model;
    model << f(0), f(1), f(2), f(3), f(4), f(5), f(6), f(7), f(8);
...
double FundamentalMatrixSevenPointEstimator::computeRMSE(
...
double FundamentalMatrixSevenPointEstimator::computeError(
...
void FundamentalMatrixSevenPointEstimator::computeError(
    const VecXData &x_data, const VecYData &y_data, const ModelType &model,
...
void FundamentalMatrixEightPointEstimator::estimate(
  ...
  Eigen::JacobiSVD<Eigen::Matrix3d> svd_F_norm(
      F_normt.transpose(), Eigen::ComputeFullU | Eigen::ComputeFullV);
  Eigen::Vector3d singular_values = svd_F_norm.singularValues();
  singular_values(2) = 0.0;
  const Eigen::Matrix3d F_norm = svd_F_norm.matrixU() *
                                 singular_values.asDiagonal() *
                                 svd_F_norm.matrixV().transpose();
...
double FundamentalMatrixEightPointEstimator::computeRMSE(
    const VecXData &x_data, const VecYData &y_data, const ModelType &model) {
  assert(x_data.size() == y_data.size());
  if (x_data.empty()) {
    return 0.0;
...
double FundamentalMatrixEightPointEstimator::computeError(
    const XDataType &x_data, const YDataType &y_data, const ModelType &model) {
  return std::sqrt(computeSquaredSampsonError(x_data, y_data, model));
}

void FundamentalMatrixEightPointEstimator::computeError(
    const VecXData &x_data, const VecYData &y_data, const ModelType &model,
    std::vector<double> &errors) {
  assert(x_data.size() == y_data.size());
