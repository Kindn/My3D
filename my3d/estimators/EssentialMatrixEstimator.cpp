...
void EssentialMatrixEightPointEstimator::estimate(const VecXData &x_data,
                                                  const VecYData &y_data,
                                                  EigenVec<ModelType> &models) {
  assert(x_data.size() >= kMinNumSamples &&
         "Data size should be greater than minimum sample number. ");
  ...
  Eigen::Matrix3d E =
      transform_y.transpose() * E_normt.transpose() * transform_x;

  /* Enforce the singularity constraint. */
  Eigen::JacobiSVD<Eigen::Matrix3d> svd_E(E, Eigen::ComputeFullU |
                                                 Eigen::ComputeFullV);
  ...
  E = svd_E.matrixU() * singular_values.asDiagonal() *
...
double EssentialMatrixEightPointEstimator::computeRMSE(const VecXData &x_data,
                                                       const VecYData &y_data,
...
double EssentialMatrixEightPointEstimator::computeError(
...
void EssentialMatrixEightPointEstimator::computeError(
    const VecXData &x_data, const VecYData &y_data, const ModelType &model,
...
void EssentialMatrixFivePointEstimator::estimate(const VecXData &x_data,
                                                 const VecYData &y_data,
                                                 EigenVec<ModelType> &models) {
  assert(x_data.size() >= kMinNumSamples &&
         "Data size should be greater than minimum sample number. ");
...
double EssentialMatrixFivePointEstimator::computeRMSE(const VecXData &x_data,
                                                      const VecYData &y_data,
                                                      const ModelType &model) {
  assert(x_data.size() == y_data.size());
  if (x_data.empty()) {
...
double EssentialMatrixFivePointEstimator::computeError(const XDataType &x_data,
                                                       const YDataType &y_data,
                                                       const ModelType &model) {
  return std::sqrt(computeSquaredSampsonError(x_data, y_data, model));
}

void EssentialMatrixFivePointEstimator::computeError(
    const VecXData &x_data, const VecYData &y_data, const ModelType &model,
    std::vector<double> &errors) {
  assert(x_data.size() == y_data.size());
...
void poseFromEssentialMatrix(const Eigen::Matrix3d &E,
  ...
  for (size_t i = 0; i < candidates_R.size(); ++i) {
    ...
    base::checkCheirality(
        candidates_R[i], candidates_t[i], Eigen::Matrix3d::Identity(),
        Eigen::Matrix3d::Identity(), points1, points2, &tmp_points_3D);
