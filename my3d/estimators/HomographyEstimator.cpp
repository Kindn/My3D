...
void HomographyEstimator::estimate(const VecXData &x_data,
  ...
  assert(x_data.size() >= kMinNumSamples &&
         "More than 4 points are needed to estimate homography. ");
  ...
  T_x.setIdentity();
  T_x.block<2, 1>(0, 2) = t_x;
  T_y.setIdentity();
  T_y.block<2, 1>(0, 2) = t_y;
  ...
  //     Eigen::Vector3d x(x_data_normalized[i].x(),
  //     x_data_normalized[i].y(), 1.0); Eigen::Vector3d
  //     y(y_data_normalized[i].x(), y_data_normalized[i].y(), 1.0); L.block<1,
  //     3>(2 * i, 0) = x.transpose(); L.block<1, 2>(2 * i, 6) = -y.x() *
  //     x.transpose().head<2>(); L.block<1, 3>(2 * i + 1, 3) = x.transpose();
  ...
  Eigen::Matrix<double, Eigen::Dynamic, 9> A =
      Eigen::Matrix<double, Eigen::Dynamic, 9>::Zero(
          2 * x_data_normalized.size(), 9);
  for (size_t i = 0; i < x_data_normalized.size(); ++i) {
  ...
  Eigen::JacobiSVD<Eigen::Matrix<double, Eigen::Dynamic, 9>> svd_A(
...
double HomographyEstimator::computeRMSE(const VecXData &x_data,
                                        const VecYData &y_data,
...
double HomographyEstimator::computeError(const XDataType &x_data,
                                         const YDataType &y_data,
                                         const ModelType &model) {
  return computeResidual(x_data, y_data, model).norm();
}

void HomographyEstimator::computeError(const VecXData &x_data,
                                       const VecYData &y_data,
                                       const ModelType &model,
...
Eigen::Vector2d HomographyEstimator::computeResidual(const XDataType &x_data,
                                                     const YDataType &y_data,
                                                     const ModelType &model) {
...
void decomposeHomography(const Eigen::Matrix3d &H0, const Eigen::Matrix3d &K1,
  ...
  const double max_abs_sii =
      std::max(std::abs(s11), std::max(std::abs(s22), std::abs(s33)));
  ...
  Eigen::Matrix3d Ra =
      H * (Eigen::Matrix3d::Identity() - 2.0 / nu * ta * na.transpose());
  Eigen::Matrix3d Rb =
      H * (Eigen::Matrix3d::Identity() - 2.0 / nu * tb * nb.transpose());
  ta = Ra * ta;
  tb = Rb * tb;
...
void poseFromHomography(const Eigen::Matrix3d &H, const Eigen::Matrix3d &K1,
  ...
  for (size_t i = 0; i < candidates_R.size(); ++i) {
    base::checkCheirality(candidates_R[i], candidates_t[i], K1, K2, points1,
                          points2, &tmp_points_3D);
