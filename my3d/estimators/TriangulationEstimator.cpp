...
void TriangulationEstimator::estimate(const VecXData &x_data, 
    ...
    if (x_data.size() == 2) {
        triangulated_point = 
            base::triangulatePoint(y_data[0].proj_matrix, y_data[1].proj_matrix, 
    ...
    for (int64_t i = 0; i < static_cast<int64_t>(y_data.size()) - 1; ++i) {
        for (int64_t j = i + 1; j < static_cast<int64_t>(y_data.size()); ++j) {
            ...
            const double tri_angle_rad = 
                base::computeTriangulationAngle(ti, 
                                                tj, 
                                                triangulated_point); 
            if (std::abs(tri_angle_rad) >= std::abs(util::deg2Rad(min_tri_angle_deg_))) {
...
double TriangulationEstimator::computeError(const XDataType &x_data, 
    ...
    if (error_type_ == EstimationErrorType::ANGULAR_ERROR) {
        return base::computeNormalizedAngularError(x_data.point_normalized, model, y_data.proj_matrix); 
    } else {
...
Eigen::Vector2d TriangulationEstimator::computeResidual(const XDataType &x_data, 
                                                        const YDataType &y_data, 
                                                        const ModelType &model) {
    return (y_data.proj_matrix * model.homogeneous()).hnormalized() - x_data.point_normalized; 
}

RobustTriangulationEstimator::Report 
estimateRobustTriangulation(const typename estimator::RANSACConfig &config, 
