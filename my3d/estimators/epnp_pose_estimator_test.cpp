...
BOOST_AUTO_TEST_CASE(TestEstimate) {
  ...
  const Eigen::Matrix3d R = base::axisAngle2RotationMatrix(
      Eigen::Vector3d(1, -1, 0), 2.0 * M_PI / 3.0);
  ...
  // std::cout << "R * R^T: \n[" << R * R.transpose() << "]" << std::endl;
  std::cout << "t_est: [" << poses[0].rightCols<1>().transpose() << "]"
            << std::endl;

  bool exists = false;
  for (size_t i = 0; i < poses.size(); ++i) {
    if ((poses[i].block<3, 3>(0, 0) * R.transpose() -
...
//     base::axisAngle2RotationMatrix(Eigen::Vector3d(1, -1, 0), 2.0 * M_PI
//     / 3.0); const Eigen::Vector3d t(3, 3, 1.2);
