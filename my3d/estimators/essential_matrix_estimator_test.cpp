...
BOOST_AUTO_TEST_CASE(TestFivePoint) {
  ...
  estimator::RANSAC<estimator::EssentialMatrixFivePointEstimator> estimator(
      config);
...
BOOST_AUTO_TEST_CASE(TestDecompose) {
  const Eigen::Matrix3d R =
      base::rpy2RotationMatrix(Eigen::Vector3d(1, 1.2, 1.3));
...
BOOST_AUTO_TEST_CASE(TestPoseFromEssentialMatrix) {
  const Eigen::Matrix3d R =
      base::rpy2RotationMatrix(util::deg2Rad(20), util::deg2Rad(10), 0);
