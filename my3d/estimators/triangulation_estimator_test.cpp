...
BOOST_AUTO_TEST_CASE(TestEstimate) {
  ...
  for (size_t i = 0; i < kNumViews; ++i) {
    ...
    const Eigen::Matrix3d R =
        base::rpy2RotationMatrix(Eigen::Vector3d(roll, pitch, yaw));
    const Eigen::Matrix3x4d proj_n =
        base::composePose(R.inverse(), -R.inverse() * t);
    const Eigen::Matrix3x4d proj = K * proj_n;
    const Eigen::Vector2d point_2D =
        (proj * point_3D.homogeneous()).hnormalized();
...
BOOST_AUTO_TEST_CASE(TestRANSAC) {
  ...
  for (size_t i = 0; i < 3; ++i) {
    x_data[i].point +=
        Eigen::Vector2d(rng.uniformReal(10, 15), rng.uniformReal(10, 15));
  ...
  std::cout << "max_inlier_error: " << ransac_config.max_inlier_error
            << std::endl;
  ...
  estimator.getEstimator().setEstimationErrorType(
      estimator::TriangulationEstimator::EstimationErrorType::
          REPROJECTION_ERROR);
