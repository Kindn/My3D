...
bool EPnPPoseEstimator::computePose(const VecXData &x_data,
...
void EPnPPoseEstimator::estimate(const VecXData &x_data, const VecYData &y_data,
...
double EPnPPoseEstimator::computeRMSE(const VecXData &x_data,
                                      const VecYData &y_data,
...
double EPnPPoseEstimator::computeError(const XDataType &x_data,
                                       const YDataType &y_data,
                                       const ModelType &model) {
  return computeResidual(x_data, y_data, model).norm();
}

void EPnPPoseEstimator::computeError(const VecXData &x_data,
                                     const VecYData &y_data,
                                     const ModelType &model,
...
Eigen::Vector2d EPnPPoseEstimator::computeResidual(const XDataType &x_data,
                                                   const YDataType &y_data,
                                                   const ModelType &model) {
...
void EPnPPoseEstimator::selectControlPoints() {
  ...
  Eigen::JacobiSVD<Eigen::Matrix3d> svd_AtA(
      A.transpose() * A, Eigen::ComputeFullU | Eigen::ComputeFullV);
  const Eigen::Vector3d evalues = svd_AtA.singularValues();
  const Eigen::Matrix3d U = svd_AtA.matrixU();
  for (size_t i = 1; i < 4; ++i) {
    cws_[i] =
        cws_[0] + std::sqrt(evalues(i - 1) / static_cast<double>(num_points)) *
                      U.col(i - 1);
  }
}

bool EPnPPoseEstimator::computeBarycentricCoordinates() {
  const size_t num_points = pws_.size();
...
void EPnPPoseEstimator::computeM() {
  const size_t num_points = pws_.size();
...
void EPnPPoseEstimator::computeVs() {
  const Eigen::Matrix<double, 12, 12> MtM = M_.transpose() * M_;
  Eigen::JacobiSVD<Eigen::Matrix<double, 12, 12>> svd_MtM(
      MtM, Eigen::ComputeFullU | Eigen::ComputeFullV);
...
void EPnPPoseEstimator::computeL6x10() {
  size_t row_idx = 0;
  for (size_t i = 0; i < 4; ++i) {
    for (size_t j = i + 1; j < 4; ++j) {
      const Eigen::Vector3d s1 =
          vs_[0].segment(i * 3, 3) - vs_[0].segment(j * 3, 3);
      const Eigen::Vector3d s2 =
          vs_[1].segment(i * 3, 3) - vs_[1].segment(j * 3, 3);
      const Eigen::Vector3d s3 =
          vs_[2].segment(i * 3, 3) - vs_[2].segment(j * 3, 3);
      const Eigen::Vector3d s4 =
          vs_[3].segment(i * 3, 3) - vs_[3].segment(j * 3, 3);
...
void EPnPPoseEstimator::computeRho() {
  size_t row_idx = 0;
  for (int i = 0; i < 4; ++i) {
    for (int j = i + 1; j < 4; ++j) {
...
// [B11 B12 B13 B14]
void EPnPPoseEstimator::solveForBetasApprox1(Eigen::Vector4d &betas) {
  ...
  Eigen::JacobiSVD<Eigen::Matrix<double, 6, 4>> svd_L6x4(
      L6x4, Eigen::ComputeFullU | Eigen::ComputeFullV);
...
// [B11 B12 B13 B14 B22 B23 B24 B33 B34 B44]
// [B11 B12         B22]
void EPnPPoseEstimator::solveForBetasApprox2(Eigen::Vector4d &betas) {
  ...
  Eigen::JacobiSVD<Eigen::Matrix<double, 6, 3>> svd_L6x3(
      L6x3, Eigen::ComputeFullU | Eigen::ComputeFullV);
  Eigen::Vector6d tmp_rho = rho_;
...
// [B11 B12 B13 B14 B22 B23 B24 B33 B34 B44]
// [B11 B12 B13     B22 B23]
void EPnPPoseEstimator::solveForBetasApprox3(Eigen::Vector4d &betas) {
  ...
  Eigen::JacobiSVD<Eigen::Matrix<double, 6, 5>> svd_L6x5(
      L6x5, Eigen::ComputeFullU | Eigen::ComputeFullV);
  Eigen::Vector6d tmp_rho = rho_;
...
void EPnPPoseEstimator::optimizeBetas(Eigen::Vector4d &betas) {
  const int kNumIterations = 10;
  double last_error = INFINITY;
  for (int iter = 0; iter < kNumIterations; ++iter) {
    // Compute residual
    Eigen::Matrix<double, 10, 1> b;
    b << betas(0) * betas(0), betas(0) * betas(1), betas(0) * betas(2),
        betas(0) * betas(3), betas(1) * betas(1), betas(1) * betas(2),
        betas(1) * betas(3), betas(2) * betas(2), betas(2) * betas(3),
        betas(3) * betas(3);
    const Eigen::Vector6d residual = L6x10_ * b - rho_;
    const double error = residual.norm();
    // std::cout << error << std::endl;
    if (error > last_error) {
      std::cout << "[EPnPPoseEstimator] Invalid Gauss-Newton step. "
                << std::endl;
      break;
    }
    last_error = error;
...
void EPnPPoseEstimator::computeCcs(const Eigen::Vector4d &betas) {
  for (int i = 0; i < 4; ++i) {
...
void EPnPPoseEstimator::computePcs() {
  const size_t num_points = pws_.size();
  pcs_.resize(num_points);
  for (size_t i = 0; i < num_points; ++i) {
    pcs_[i] = bcs_[i](0) * ccs_[0] + bcs_[i](1) * ccs_[1] +
              bcs_[i](2) * ccs_[2] + bcs_[i](3) * ccs_[3];
  }
}

void EPnPPoseEstimator::solveForSign() {
  if (pcs_.front().z() < 0.0) {
    for (int i = 0; i < 4; ++i) {
      ccs_[i] = -ccs_[i];
    }
...
void EPnPPoseEstimator::estimatePose(Eigen::Matrix3x4d &pose) {
  const size_t num_points = pws_.size();
  ...
  Eigen::JacobiSVD<Eigen::Matrix3d> svd_W(W, Eigen::ComputeFullU |
                                                 Eigen::ComputeFullV);
