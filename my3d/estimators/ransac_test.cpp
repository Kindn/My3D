...
BOOST_AUTO_TEST_CASE(TestCountInliers) {
  ...
  my3d::estimator::RANSAC<typename my3d::estimator::HomographyEstimator> ransac(
      config);

  std::vector<bool> inlier_mask;
  std::vector<double> errors;
  size_t num_inliers =
      ransac.countInliers(x_data, y_data, model, inlier_mask, errors);
...
BOOST_AUTO_TEST_CASE(TestHomography) {
  ...
  my3d::estimator::RANSAC<typename my3d::estimator::HomographyEstimator>::Report
      report = ransac.estimate(x_data, y_data);
