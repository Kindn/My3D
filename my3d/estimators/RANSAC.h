...
RANSAC<EstimatorType>::RANSAC(const RANSACConfig &config) {
    ...
    config_ = config;
    const size_t dyn_max_iter_num = computeDynamicMaxIterNum(config_.min_inlier_ratio); 
    config_.max_iter_num = std::min<size_t>(dyn_max_iter_num, config_.max_iter_num); 
}

template <typename EstimatorType>
size_t 
RANSAC<EstimatorType>::countInliers(const typename EstimatorType::VecXData &x_data, 
                                    const typename EstimatorType::VecYData &y_data, 
                                    const typename EstimatorType::ModelType &model, 
                                    std::vector<bool> &inlier_mask, 
                                    std::vector<double> &errors) {
...
template <typename EstimatorType>
double 
RANSAC<EstimatorType>::computeInlierRMSE(const typename EstimatorType::VecXData &x_data, 
                                         const typename EstimatorType::VecYData &y_data, 
                                         const std::vector<bool> &inlier_mask, 
                                         const typename EstimatorType::ModelType &model) {
...
template <typename EstimatorType>
double 
RANSAC<EstimatorType>::computeInlierErrorSum(const typename EstimatorType::VecXData &x_data, 
                                             const typename EstimatorType::VecYData &y_data, 
                                             const std::vector<bool> &inlier_mask, 
                                             const typename EstimatorType::ModelType &model) {
...
template <typename EstimatorType>
void 
RANSAC<EstimatorType>::sampleMinimumPointSet(size_t num_points,  
                                             std::vector<size_t> &sampled_idx) {
...
template <typename EstimatorType>
size_t RANSAC<EstimatorType>::computeDynamicMaxIterNum(const double &inlier_ratio) {
    const double nom = 1.0 - config_.confidence; 
    if (nom <= 0) {
        return std::numeric_limits<size_t>::max(); 
    } 
    const double denom = 1.0 - std::pow(inlier_ratio, static_cast<double>(EstimatorType::kMinNumSamples)); 
    if (denom <= 0) {
        return 1; 
    } else if (denom == 1.0) {
        return std::numeric_limits<size_t>::max(); 
...
template <typename EstimatorType>
typename RANSAC<EstimatorType>::Report 
RANSAC<EstimatorType>::estimateBySampling(const typename EstimatorType::VecXData &x_data, 
                                          const typename EstimatorType::VecYData &y_data) {
    assert(x_data.size() == y_data.size());
    assert(x_data.size() >= EstimatorType::kMinNumSamples);
    ...
    while (report.num_iter < config_.max_iter_num) {
        ...
        for (const auto &model : models) {
            std::vector<bool> inlier_mask;
            std::vector<double> errors; 
            const size_t num_inliers = countInliers(x_data, y_data, model, inlier_mask, errors);
            const double inlier_ratio = static_cast<double>(num_inliers) / static_cast<double>(num_points); 
            const double inlier_error = computeInlierRMSE(x_data, y_data, inlier_mask, model);
            const double inlier_error_sum = computeInlierErrorSum(x_data, y_data, inlier_mask, model); 
            dyn_max_iter_num = computeDynamicMaxIterNum(inlier_ratio); 
                // static_cast<size_t>(std::log(1.0 - config_.confidence) / 
                // std::log(1.0 - std::pow(inlier_ratio, static_cast<double>(EstimatorType::kMinNumSamples))));
            dyn_max_iter_num = std::max(dyn_max_iter_num, config_.min_iter_num);
            // std::cout << "inlier number: " << num_inliers << ", inlier ratio: " << inlier_ratio << ", " 
            //           << ", error: " << inlier_error << ", min error: " << best_inlier_error << ", "
            //           << report.num_iter << ", " << dyn_max_iter_num << std::endl;
            // std::cout << model << std::endl;
            if (num_inliers > max_num_inliers || 
                (num_inliers == max_num_inliers && inlier_error_sum < best_inlier_error_sum)) {
        ...
        report.num_iter++;
        if (config_.verbose) {
            std::cout << "\r[RANSAC] iterations: " << report.num_iter << "/" << config_.max_iter_num;
        }
...
template <typename EstimatorType>
typename RANSAC<EstimatorType>::Report 
RANSAC<EstimatorType>::estimateByExhaustion(const typename EstimatorType::VecXData &x_data, 
                                          const typename EstimatorType::VecYData &y_data) { 
    Report report; 
