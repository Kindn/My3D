...
public: 
    ...
    LocalEstimatorType &getLocalEstimator() {
        return local_estimator_; 
    }
...
LORANSAC<EstimatorType, LocalEstimatorType>::estimate(const typename EstimatorType::VecXData &x_data, 
                                                      const typename EstimatorType::VecYData &y_data, 
                                                      const bool &exhaust) {
    ...
    while (report.num_iter < this->config_.max_iter_num)
    {
        ...
        for (const auto &model : models) {
            ...
            const size_t num_inliers = this->countInliers(x_data, y_data, model, inlier_mask, errors);
            const double inlier_ratio = static_cast<double>(num_inliers) / static_cast<double>(num_points); 
            const double inlier_error = this->computeInlierRMSE(x_data, y_data, inlier_mask, model);
            const double inlier_error_sum = this->computeInlierErrorSum(x_data, y_data, inlier_mask, model); 
                ...
                if (num_inliers > EstimatorType::kMinNumSamples && 
                    ...
                    for (size_t iter = 0; iter < kMaxNumLocalIters; ++iter) {
                        ...
                        for (const auto &local_model : local_models) {
                            ...
                            const size_t num_local_inliers = this->countInliers(x_data, y_data, local_model, local_inlier_mask, local_errors); 
                            const double local_inlier_error = this->computeInlierRMSE(x_data, y_data, local_inlier_mask, local_model); 
                            const double local_inlier_error_sum = this->computeInlierErrorSum(x_data, y_data, local_inlier_mask, local_model); 
                            if (num_local_inliers > max_num_inliers || 
                                (num_local_inliers == max_num_inliers && local_inlier_error_sum < best_inlier_error_sum)) {
                                best_inlier_error_sum = inlier_error_sum; 
        ...
        if (this->config_.verbose) {
            std::cout << "\r[RANSAC] iterations: " << report.num_iter << "/" << this->config_.max_iter_num;
        }
