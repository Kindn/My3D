  ...
  for (double x = 1; x <= 10; ++x) {
    ...
    for (size_t i = 0; i < 4; ++i) {
      Eigen::Vector2d residual =
          estimator.computeResidual(src[i], dst[i], result_model);
...
BOOST_AUTO_TEST_CASE(TestEstimate2) {
  ...
  for (size_t i = 0; i < num_points; ++i) {
    x_data.push_back(
        Eigen::Vector2d(rng.uniformReal(-10, 10), rng.uniformReal(-10, 10)));
    ...
    for (size_t i2 = i1 + 1; i2 < num_points - 2UL; ++i2) {
      for (size_t i3 = i2 + 1; i3 < num_points - 1UL; ++i3) {
        for (size_t i4 = i3 + 1; i4 < num_points; ++i4) {
          ...
          BOOST_CHECK_LT((models[0].normalized() - model.normalized()).norm(),
                         0.1);
          // std::cout << "[" << result_model << "]" << std::endl;
          // for (size_t i = 0; i < num_points; ++i) {
          //     Eigen::Vector2d residual = estimator.computeResidual(x_data[i],
          //     y_data[i], result_model); BOOST_CHECK_LT(residual.norm(),
          //     1e-3);
          // }
        }
      }
...
BOOST_AUTO_TEST_CASE(TestEstimateRandomData) {
  ...
  for (double x = 1; x <= 10; ++x) {
    ...
    VecXData src;
    for (size_t i = 0; i < 4; ++i) {
      src.emplace_back(rng.uniformReal(-1000, 1000),
...
BOOST_AUTO_TEST_CASE(TestDecompose) {
  ...
  for (size_t i = 0; i < 4; ++i) {
    ...
    std::cout << "[" << R[i] << "], [" << t[i].transpose() << "], ["
              << n[i].transpose() << "]" << std::endl;
...
BOOST_AUTO_TEST_CASE(TestPoseFromHomography) {
  ...
  my3d::estimator::poseFromHomography(H, K1, K2, points1, points2, R, t, n,
                                      points3D);
