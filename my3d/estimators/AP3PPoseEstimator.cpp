...
int AP3PPoseEstimator::computePose(const VecXData &x_data,
  ...
  if ((pG1 - pG2).norm() < epsilon_ || (bc1.cross(bc2)).norm() < epsilon_ ||
    ...
    // std::cout << "[" << __PRETTY_FUNCTION__ << "] " << "Warning: Singularity
    // detected. " << std::endl;
    return 0;
  }
  ...
  bool success_solving =
      base::solvePolynomialCompanionMatrix(coeffs, roots_real, roots_imag);
  if (!success_solving) {
    std::cout << "[" << __PRETTY_FUNCTION__ << "] "
              << "Warning: Solving polynomial failed. " << std::endl;
    return 0;
  } else {
    bool success_refinement = base::refineRoots(coeffs, roots_real, 2u);
    if (!success_refinement) {
      std::cout << "[" << __PRETTY_FUNCTION__ << "] "
                << "Warning: Root refinement failed. " << std::endl;
      return 0;
    }
  }
  ...
  for (Eigen::Index i = 0; i < roots_real.size(); ++i) {
    if (std::abs(roots_real(i)) >
        1.0 -
            std::numeric_limits<
                double>::epsilon()) { // || std::abs(roots_imag(i)) > epsilon) {
      continue;
    }
    ...
    if (std::isnan(cos_theta1p) || std::isnan(sin_theta1p)) {
      std::cout << "[" << __PRETTY_FUNCTION__ << "] "
                << "Warning: NaN occurs. " << std::endl;
      return 0;
    }
    // Compute sin(theta_1') and cos(theta_1')
    const double factor =
        sin_theta1p / (g5 * cos2_theta1p + g6 * cos_theta1p + g7);
    ...
    R_x_theta1p << 1, 0, 0, 0, cos_theta1p, sin_theta1p, 0, -sin_theta1p,
        cos_theta1p;
    R_y_theta3p << cos_theta3p, 0, -sin_theta3p, 0, 1, 0, sin_theta3p, 0,
        cos_theta3p;
...
void AP3PPoseEstimator::estimate(const VecXData &x_data, const VecYData &y_data,
...
double AP3PPoseEstimator::computeRMSE(const VecXData &x_data,
                                      const VecYData &y_data,
...
double AP3PPoseEstimator::computeError(const XDataType &x_data,
                                       const YDataType &y_data,
                                       const ModelType &model) {
  return computeResidual(x_data, y_data, model).norm();
}

void AP3PPoseEstimator::computeError(const VecXData &x_data,
                                     const VecYData &y_data,
                                     const ModelType &model,
...
Eigen::Vector2d AP3PPoseEstimator::computeResidual(const XDataType &x_data,
                                                   const YDataType &y_data,
                                                   const ModelType &model) {
