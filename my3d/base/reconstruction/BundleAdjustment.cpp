...
VertexCamera::VertexCamera(const typename my3d::base::BundleAdjustment::Camera &camera)
{
    this->setEstimate(camera); 
}

VertexCamera::VertexCamera(const size_t &id, 
...
fix_intrinsics_{false}, fix_translation_{fix_translation}, fix_rotation_{fix_rotation} {
    this->setId(id); 
    this->setEstimate(camera); 
    this->setFixed(fixed);
}
...
void VertexCamera::plus(const Eigen::VectorXd &update) {
    if (!fix_rotation_) {
        estimate_.rotation = (estimate_.rotation * Eigen::Quaterniond(1.0, update(0) / 2.0, update(1) / 2.0, update(2) / 2.0)).normalized();
    }
    if (!fix_translation_) {
...
VertexCameraPose::VertexCameraPose(const typename my3d::base::BundleAdjustment::Camera &camera)
{
    this->setEstimate(camera); 
}

VertexCameraPose::VertexCameraPose(const size_t &id, 
                            const typename my3d::base::BundleAdjustment::Camera &camera, 
                            const bool &fixed) {
    this->setId(id); 
    this->setEstimate(camera); 
...
VertexCameraTranslation::VertexCameraTranslation(const typename my3d::base::BundleAdjustment::Camera &camera) {
    this->setEstimate(camera); 
}

VertexCameraTranslation::VertexCameraTranslation(const size_t &id, 
                            const typename my3d::base::BundleAdjustment::Camera &camera, 
                            const bool &fixed) {
    this->setId(id); 
    this->setEstimate(camera); 
...
void VertexCameraTranslation::setToOrigin() {
    estimate_.translation.setZero();
}
...
VertexCameraRotation::VertexCameraRotation(const typename my3d::base::BundleAdjustment::Camera &camera) {
    this->setEstimate(camera); 
}

VertexCameraRotation::VertexCameraRotation(const size_t &id, 
                                           const typename my3d::base::BundleAdjustment::Camera &camera, 
                                           const bool &fixed) {
    this->setId(id); 
    this->setEstimate(camera); 
...
void VertexCameraRotation::setToOrigin() {
    estimate_.rotation.setIdentity();
}
...
VertexCameraIntrinsics::VertexCameraIntrinsics(const typename my3d::base::BundleAdjustment::Camera &camera) {
    this->setEstimate(camera); 
}

VertexCameraIntrinsics::VertexCameraIntrinsics(const size_t &id, 
                                           const typename my3d::base::BundleAdjustment::Camera &camera, 
                                           const bool &fixed) {
    this->setId(id); 
    this->setEstimate(camera); 
...
void VertexPoint3D::setToOrigin() {
    estimate_.setZero();
}
...
void EdgeProjectionFull::computeResidual() {
    ...
    const typename my3d::base::BundleAdjustment::Camera camera = v0->getEstimate();
    const Eigen::Vector3d &point_3D = v1->getEstimate();
    const Eigen::Vector3d pc = camera.rotation.inverse() * (point_3D - camera.translation); 
    ...
    // residual_ = my3d::base::computeReprojectionResidual(point_3D, measurement_, pose, &camera.camera); 
}
...
void EdgeProjectionSeparated::computeResidual() {
    ...
    const typename my3d::base::BundleAdjustment::Camera camera = v_in->getEstimate();
...
bool BundleAdjustment::Camera::hasBogusParams(const double &min_focal_length_ratio, 
...
BundleAdjustment::BundleAdjustment(const Config &config): 
config_{config} {
...
bool BundleAdjustment::addObservation(size_t id_point_3D, size_t id_camera, const Eigen::Vector2d &observation) {
    ...
    point_3D->observations.insert(std::make_pair(id_camera, camera->points_2D.size() - 1UL)); 

    return true;
}

bool BundleAdjustment::addObservation(size_t id_point_3D, size_t id_camera, size_t observation_idx) {
    if (cameras_.find(id_camera) == cameras_.end() || 
        points_3D_.find(id_point_3D) == points_3D_.end()) {
        return false;
    }
...
void BundleAdjustment::config(const Config &config) {
    config_ = config;
}

BundleAdjustment::Report 
BundleAdjustment::optimize() {
    Report report; 
    ...
    for (auto &camera : cameras_) {
        ...
        case CameraOptimizationMode::FULL: {
            ...
            v = std::make_shared<gopt::VertexCamera>(id_vertex, *(camera.second), 
                camera.second->fixed, 
                camera.second->fix_intrinsics, camera.second->fix_translation, camera.second->fix_rotation); 
            if (!graph.addVertex(v)) {
                report.success = false;
                report.error_info = "Failed to add vertex for camera " + std::to_string(camera.first);
        ...
        case CameraOptimizationMode::ONLY_POSE: {
            gopt::FactorGraph::VertexPtr v; 
            v = std::make_shared<gopt::VertexCameraPose>(id_vertex, *(camera.second), camera.second->fixed); 
            if (!graph.addVertex(v)) {
                report.success = false;
                report.error_info = "Failed to add vertex for camera " + std::to_string(camera.first);
        ...
        case CameraOptimizationMode::FULL_AND_SEPARATED: {
            gopt::FactorGraph::VertexPtr v_in = 
                std::make_shared<gopt::VertexCameraIntrinsics>(id_vertex, *(camera.second), camera.second->fix_intrinsics); 
            gopt::FactorGraph::VertexPtr v_t = 
                std::make_shared<gopt::VertexCameraTranslation>(id_vertex, *(camera.second), camera.second->fix_translation); 
            gopt::FactorGraph::VertexPtr v_r = 
                std::make_shared<gopt::VertexCameraRotation>(id_vertex, *(camera.second), camera.second->fix_rotation); 
            if (!(graph.addVertex(v_in) && 
                graph.addVertex(v_t) && 
                graph.addVertex(v_r))) {
                report.success = false;
                report.error_info = "Failed to add vertex for camera " + std::to_string(camera.first);
            ...
            gopt_vertices_camera_intrinsics_.insert(std::make_pair(camera.first, v_in)); 
            gopt_vertices_camera_translation_.insert(std::make_pair(camera.first, v_t)); 
    ...
    for (auto &point_3D : points_3D_) {
        ...
        if (!graph.addVertex(v)) {
            report.success = false;
            report.error_info = "Failed to add vertex for 3D point " + std::to_string(point_3D.first);
    ...
    for (auto &point_3D : points_3D_) {
        ...
        if (gopt_vertices_point_3D_.find(id_point_3D) == gopt_vertices_point_3D_.end()) {
            continue;
        }
        ...
        for (auto &observation : point_3D.second->observations) {
            ...
            if (gopt_vertices_camera_.find(id_camera) != gopt_vertices_camera_.end()) {
                if (idx_point_2D >= cameras_.at(id_camera)->points_2D.size()) {
                    continue;
                }

                const Eigen::Vector2d measurement = cameras_.at(id_camera)->points_2D[idx_point_2D];
                ...
                const std::shared_ptr<gopt::LossFunctionBase> loss = std::make_shared<gopt::HuberLoss>(); 
                switch (config_.cam_opt_mode)
                {
                case CameraOptimizationMode::FULL: 
                    edge = std::make_shared<gopt::EdgeProjectionFull>(id_edge, 
                        gopt_vertices_camera_.at(id_camera), gopt_vertices_point_3D_.at(id_point_3D), 
                        measurement, Eigen::Matrix2d::Identity(), loss); 
                    break;
                
                case CameraOptimizationMode::ONLY_POSE: 
                    edge = std::make_shared<gopt::EdgeProjectionOnlyPose>(id_edge, 
                        gopt_vertices_camera_.at(id_camera), gopt_vertices_point_3D_.at(id_point_3D), 
                        measurement, Eigen::Matrix2d::Identity(), loss); 
                    break; 

                case CameraOptimizationMode::FULL_AND_SEPARATED: 
                    edge = std::make_shared<gopt::EdgeProjectionSeparated>(id_edge, 
                        gopt_vertices_camera_intrinsics_.at(id_camera), 
                        gopt_vertices_camera_translation_.at(id_camera), 
                        gopt_vertices_camera_rotation_.at(id_camera), 
                        gopt_vertices_point_3D_.at(id_point_3D), 
                        measurement, Eigen::Matrix2d::Identity(), loss); 
                    break; 
                ...
                if (!graph.addEdge(edge)) {
                    report.success = false; 
                    report.error_info = "Failed to add edge for camera " + std::to_string(id_camera) + 
                                        " and 3D point " + std::to_string(id_point_3D); 

                    return report;
                }
                gopt_edges_projection_.push_back(EdgeInfo(id_camera, id_point_3D, edge));
                id_edge++;
            }
        }
    }

    std::cout << "[BundleAdjustmentINFO] " << "Processing " << gopt_vertices_camera_.size() << " cameras, " 
...
void BundleAdjustment::update() {
    for (auto &v : gopt_vertices_camera_) {
        ...
        case CameraOptimizationMode::FULL:
            *camera = 
                std::dynamic_pointer_cast<gopt::VertexCamera>(v.second)->getEstimate(); 
            break;
        
        case CameraOptimizationMode::ONLY_POSE: 
            *camera = 
                std::dynamic_pointer_cast<gopt::VertexCameraPose>(v.second)->getEstimate(); 
            break;

        case CameraOptimizationMode::FULL_AND_SEPARATED: 
            *camera = 
                std::dynamic_pointer_cast<gopt::VertexCameraIntrinsics>(v.second)->getEstimate(); 
            camera->id = v.first; 
            camera->rotation = 
                std::dynamic_pointer_cast<gopt::VertexCameraRotation>(v.second)->getEstimate().rotation; 
            camera->translation = 
                std::dynamic_pointer_cast<gopt::VertexCameraTranslation>(v.second)->getEstimate().translation; 
            break; 
