...
public:
  Reconstruction(base::Scene *scene) : scene_{scene} {
...
public:
  ...
  inline void reset() { reset(scene_); }

  inline void clearTracks() { tracks_.clear(); }

  inline void clearRegisteredImageIDs() { registered_image_ids_.clear(); }

  inline void clearFilteredImageIDs() { filtered_image_ids_.clear(); }

  inline base::Scene *getScene() { return scene_; }

  inline const base::Scene *getScene() const { return scene_; }
  ...
  inline size_t getRefImageID0() const { return ref_image_id_0_; }

  /**
   * @brief Note: The existence and registration of the id is guaranteed by the
   * user
   */
  ...
  inline size_t getRefImageID1() const { return ref_image_id_1_; }
  ...
  /**
   * @brief Note: The existence and registration of the ids is guaranteed by the
   * user
   */
  inline void
  setInitialImagePair(const std::pair<size_t, size_t> &initial_image_pair) {
    initial_image_pair_ = initial_image_pair;
  }

  inline size_t getInitialEdgeID() const { return initial_edge_id_; }
  ...
  /**
   * @brief Add an observation. This function will do nothing if
   * images_[image_id] or tracks_.at(track_id) does not exist.
   */
  void addObservation(size_t track_id, size_t image_id, size_t observation_idx);

  /**
   * @brief Remove an observation. This function will do nothing if
   * images_[image_id] or tracks_.at(track_id) does not exist.
   */
  void removeObservation(size_t track_id, size_t image_id,
  ...
  size_t addTrack(const base::TrackElementList &elements,
                  const Eigen::Vector3d &position, bool is_triangulated = true);

  /**
   * @brief Remove a track. This function will do nothing if the track with
   ...
   * This function will do nothing if images_[image_id] or tracks_.at(track_id)
   * does not exist.
   */
   ...
   */
  void transform(const double &scale, const Eigen::Quaterniond &rotation,
                 const Eigen::Vector3d &translation);

  inline bool hasTrack(const size_t &image_id, const size_t &observation_idx,
  ...
  inline void clearModifiedTrackIDs() { modified_track_ids_.clear(); }

  inline void addNewTrackID(const size_t &track_id) {
    new_track_ids_.insert(track_id);
  }

  inline void clearNewTrackIDs() { new_track_ids_.clear(); }

  inline void addNewImageID(const size_t &image_id) {
    new_image_ids_.insert(image_id);
  }

  inline void clearNewImageIDs() { new_image_ids_.clear(); }
