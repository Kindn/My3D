...
public: 
    ...
    struct Config {
        ...
        OptAlgorithm optimization_algorithm{OptAlgorithm::ALGORITHM_LEVENBERG_MARQUART_SPARSE_SHUR};
        bool verbose{true};
    ...
    struct EdgePair {
        EdgePair() {}
        EdgePair(size_t idc, size_t idp): 
        id_camera(idc), id_point_3D(idp) {}
    ...
    struct EdgeInfo {
        EdgeInfo() {}
        EdgeInfo(size_t idc, size_t idp, const std::shared_ptr<gopt::FactorGraph::Edge> e): 
...
public: 
    ...
    /**
     * @brief Add an camera into the BA problem. The function will do nothing and return
     *  false if 
     * 1) there already exists a camera with the same ID 
     * 2) camera == nullptr
     */
     ...
     * 1) there already exists a camera with the same ID 
     */
    bool addCamera(const size_t &id, 
                   const size_t &width, 
                   const size_t &height, 
                   ...
                   const base::RadialPinHoleCamera camera_model = base::RadialPinHoleCamera()) {
    ...
    /**
     * @brief Add an 3D point into the BA problem. The function will do nothing and return
     *  false if there already exists a point with the same ID. 
     */
    ...
    bool addPoint3D(size_t id, 
                    ...
                    const std::unordered_map<size_t, size_t> &observations = std::unordered_map<size_t, size_t>(), 
                    const bool &fixed = false) {
    ...
    /**
     * @brief Add an observation into the BA problem. The function will do nothing and return
     *  false if the specified 3D point or camera does not exist. Note that this function 
     * will not check the repetition of observations in the specified camera. 
     */
    bool addObservation(size_t id_point_3D, size_t id_camera, const Eigen::Vector2d &observation); 

    /**
     * @brief Add an observation into the BA problem. The function will do nothing and return
     *  false if the specified 3D point or camera does not exist, or observation_idx exceeds 
     * the limit
     */
    bool addObservation(size_t id_point_3D, size_t id_camera, size_t observation_idx); 

    /**
     * @brief Remove the node with ID id. If the node with the given ID does not exist, 
     * this function will do nothing. Note that this function will not remove the observations 
     * corresponding to the removed camera in 3D points. 
     */
    ...
    /**
     * @brief Remove an observation into the BA problem. The function will do nothing  if the 
     * specified 3D point or camera does not exist. 
     */
    ...
    */
    ...
    /**
     * @brief Solve the BA problem constructed by the cameras, 3D points, and corresponding observations 
     * you add. 
    */
    Report optimize();
    ...
    */
    ...
    /**
     * @brief Update camera parameters and 3D point coordinates to cameras_ and points_3D_ after 
     * optimization
    */
    void update();
    ...
    */
    const std::unordered_map<size_t, Camera*> &getCameras() const { return cameras_; } 

    /**
     * @brief Get the 3D points (const version)
    */
    const std::unordered_map<size_t, Point3D*> &getPoints3D() const { return points_3D_; } 

    /**
     * @brief Only should be called if camera optimization mode is FULL(_AND_SEPARATED) 
    */
    void fixCameraIntrinsics(const size_t &camera_id) {
        if (config_.cam_opt_mode == CameraOptimizationMode::FULL_AND_SEPARATED && 
            gopt_vertices_camera_intrinsics_.find(camera_id) != gopt_vertices_camera_intrinsics_.end()) {
    ...
    */
    void fixCameraTranslation(const size_t &camera_id) {
        if (config_.cam_opt_mode == CameraOptimizationMode::FULL_AND_SEPARATED && 
            gopt_vertices_camera_translation_.find(camera_id) != gopt_vertices_camera_translation_.end()) {
    ...
    */
    void fixCameraRotation(const size_t &camera_id) {
        if (config_.cam_opt_mode == CameraOptimizationMode::FULL_AND_SEPARATED && 
            gopt_vertices_camera_rotation_.find(camera_id) != gopt_vertices_camera_rotation_.end()) {
...
private:
    ...
    std::unordered_map<size_t, std::shared_ptr<gopt::FactorGraph::Vertex>> gopt_vertices_camera_;
    std::unordered_map<size_t, std::shared_ptr<gopt::FactorGraph::Vertex>> gopt_vertices_camera_intrinsics_;
    std::unordered_map<size_t, std::shared_ptr<gopt::FactorGraph::Vertex>> gopt_vertices_camera_translation_;
    std::unordered_map<size_t, std::shared_ptr<gopt::FactorGraph::Vertex>> gopt_vertices_camera_rotation_;
    std::unordered_map<size_t, std::shared_ptr<gopt::VertexPoint3D>> gopt_vertices_point_3D_;
...
/**
 * @brief Vertex representing a camera. This class is not responsible for managing the memory 
 * to which the Camera pointer points. 
 * 
*/
template <size_t Dim> 
struct VertexCameraBase : public gopt::BaseVertex<Dim, typename my3d::base::BundleAdjustment::Camera>  {
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
...
struct VertexCameraTranslation : public VertexCameraBase<3> {
    ...
    VertexCameraTranslation(const typename my3d::base::BundleAdjustment::Camera &camera); 
    VertexCameraTranslation(const size_t &id, 
                           const typename my3d::base::BundleAdjustment::Camera &camera, 
                           const bool &fixed);
...
struct VertexCameraRotation : public VertexCameraBase<3> {
    ...
    VertexCameraRotation(const typename my3d::base::BundleAdjustment::Camera &camera); 
    VertexCameraRotation(const size_t &id, 
                           const typename my3d::base::BundleAdjustment::Camera &camera, 
                           const bool &fixed);
...
struct VertexCameraIntrinsics : public VertexCameraBase<6> {
    ...
    VertexCameraIntrinsics(const typename my3d::base::BundleAdjustment::Camera &camera); 
    VertexCameraIntrinsics(const size_t &id, 
                           const typename my3d::base::BundleAdjustment::Camera &camera, 
                           const bool &fixed);
...
struct EdgeProjectionFull : 
    public gopt::BaseBinaryEdge<2, Eigen::Vector2d, VertexCamera, VertexPoint3D> {
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
...
struct EdgeProjectionOnlyPose : 
    public gopt::BaseBinaryEdge<2, Eigen::Vector2d, VertexCameraPose, VertexPoint3D> {
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW 
...
struct EdgeProjectionSeparated : 
    public gopt::BaseEdge<2, Eigen::Vector2d> {
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW 

    EdgeProjectionSeparated(); 
    // The camera pointer, i.e. the member estimator_ of the 3 camera vertices should be the same 
    EdgeProjectionSeparated(const size_t &id, 
