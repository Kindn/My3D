...
Scene::CameraInfo::CameraInfo()
    : rotation{Eigen::Quaterniond::Identity()}, translation{
                                                    Eigen::Vector3d::Zero()} {}
...
Scene::TwoViewGeometryInfo::TwoViewGeometryInfo()
    : fundamental_matrix{Eigen::Matrix3d::Identity()},
      ...
      R{Eigen::Matrix3d::Identity()}, t{Eigen::Vector3d::Zero()} {}
...
Scene::Node::Node(size_t _id, int _width, int _height,
    ...
    : id{_id}, width{_width}, height{_height}, points_2D{_points_2D},
      connected_node_ids{_connected_node_ids} {}

Eigen::Matrix3x4d Scene::Node::getProjectionMatrix() const {
  const Eigen::Matrix3d rotation =
      base::quaternion2RotationMatrix(getRotation());
  const Eigen::Vector3d translation = getTranslation();
  return getCameraMatrix() *
         base::composePose(rotation.inverse(),
...
double Scene::Node::computeReprojectionError(const Eigen::Vector3d &point_3D,
  ...
  // const double error1 =  base::computeReprojectionError(point_3D, point_2D,
  // proj_matrix);
  const base::CameraBase *camera = new base::RadialPinHoleCamera(
      getCameraMatrix(), camera_info.k1, camera_info.k2);
  const Eigen::Matrix3d rotation =
      base::quaternion2RotationMatrix(getRotation());
  const Eigen::Vector3d translation = getTranslation();
  const Eigen::Matrix3x4d pose =
      base::composePose(rotation.inverse(), -rotation.inverse() * translation);
  const double error =
      base::computeReprojectionError(point_3D, point_2D, pose, camera);
  delete camera;
  // std::cout << "(" << error1 << ", " << error2 << std::endl;
  return error;
}

bool Scene::Node::hasBogusCameraParams(
  ...
  bool has_bogus_principal_points =
      camera_info.cx < 0.0 || camera_info.cx > width || camera_info.cy < 0.0 ||
      camera_info.cy > height;
...
bool Scene::addNode(const size_t &id, const size_t &width, const size_t &height,
  ...
  Node *node =
      new Node(id, width, height, points_2D, std::unordered_set<size_t>());
...
bool Scene::addEdge(
  ...
  if (std::find(node1->connected_node_ids.begin(),
                node1->connected_node_ids.end(),
                node_ids[1]) != node1->connected_node_ids.end()) {
    return false;
  }
...
void Scene::addObservation(const size_t &track_id, const size_t &node_id,
...
void Scene::removeObservation(const size_t &track_id, const size_t &node_id) {
  if (!existsNode(node_id)) {
    return;
  }
...
bool Scene::checkTrackDirectVisibilityInNode(const base::Track &track,
  ...
  for (auto &element : track.getElements()) {
    ...
    /* Check whether the 2D points in the given track have correspondences with
       the given node */
    if (!existsNode(element.image_id)) {
      continue;
    } else {
      const base::Scene::Node *const node_in_track =
          nodes_.at(element.image_id);
      ...
      for (auto &connected_edge : node_in_track->connected_edges) {
        ...
        if (edge->node_ids[0] == element.image_id &&
            edge->node_ids[1] == node_id) {
        ...
        } else if (edge->node_ids[1] == element.image_id &&
                   edge->node_ids[0] == node_id) {
          for (auto &corr : edge->correspondences) {
    ...
    if (edge.second->node_ids[0] == node_id) {
      for (auto &corr : edge.second->correspondences) {
        if (corr.first == feature_idx) {
          correspondences.push_back(
              std::make_pair(edge.second->node_ids[1], corr.second));
        }
      }
      ...
      for (auto &corr : edge.second->correspondences) {
        if (corr.second == feature_idx) {
          correspondences.push_back(
              std::make_pair(edge.second->node_ids[0], corr.first));
        }
      }
    }
  }
  ...
  for (size_t level = 0; level < max_transitivity; ++level) {
    for (size_t i = last_level_begin; i < last_level_end; ++i) {
      ...
      for (const auto &current_corr : current_corrs) {
        ...
        if (image_corrs[current_corr_image_id]
                .insert(current_corr_feature_idx)
                .second) {
          correspondences.emplace_back(current_corr_image_id,
                                       current_corr_feature_idx);
        }
      }
    }
...
bool Scene::hasTrack(const size_t &node_id, const size_t &feature_idx,
                     size_t *track_id) const {
  if (!existsNode(node_id)) {
    std::cout << "[WARNING] " << __PRETTY_FUNCTION__ << "Node " << node_id
...
bool Scene::hasNodeBogusCameraParameters(
    const size_t &node_id, const double &min_focal_length_ratio,
    const double &max_focal_length_ratio) const {
  if (!existsNode(node_id)) {
    return true;
  }
  ...
  return node->hasBogusCameraParams(min_focal_length_ratio,
...
bool Scene::isTwoViewObservation(const size_t &node_id,
  ...
  const auto corrs1 = findDirectCorrespondences(node_id, point_2D_idx);
  if (corrs1.size() != 1) {
    return false;
  }
