...
bool Reconstruction::registerInitialImagePair(
  ...
  if (scene_->existsNode(initial_pair.first) &&
    ...
    if (initial_edge->node_ids[0] == initial_pair.first &&
...
void Reconstruction::deregisterImage(const size_t &image_id) {
  if (!existsImage(image_id) || !isImageRegistered(image_id) ||
...
void Reconstruction::addObservation(size_t track_id, size_t image_id,
...
void Reconstruction::removeObservation(size_t track_id, size_t image_id,
                                       size_t observation_idx) {
  if (existsTrack(track_id) && existsImage(image_id)) {
...
void Reconstruction::normalize(const double &extent, const double &p0,
  ...
  const double scale = (old_extent < std::numeric_limits<double>::epsilon())
                           ? 1.0
                           : (extent / old_extent);
...
void Reconstruction::transform(const double &scale,
  ...
  for (const size_t &image_id : registered_image_ids_) {
    ...
    const Eigen::Vector3d pos =
        scale * (rotation.normalized().toRotationMatrix() * old_pos) +
        translation;
    const Eigen::Quaternion rot =
        (rotation.normalized() * old_rot).normalized();
