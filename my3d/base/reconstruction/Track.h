...
public:
  ...
  inline bool isTriangulated() const { return is_triangulated_; }

  inline void setTriangulated(const bool &is_triangulated) {
    is_triangulated_ = is_triangulated;
  }

  inline void set3DPosition(const Eigen::Vector3d &pos_3d) { pos_3d_ = pos_3d; }

  inline Eigen::Vector3d get3DPosition() const { return pos_3d_; }
  ...
  inline void setId(const size_t &id) { id_ = id; }

  inline size_t getId() const { return id_; }

  inline TrackElementList &getElements() { return elements_; }

  inline const TrackElementList &getElements() const { return elements_; }

  inline void setElements(const TrackElementList &elements) {
    elements_ = elements;
  }

  inline bool existsElement(const size_t &image_id,
                            const size_t &feature_idx) const {
    for (const auto &element : elements_) {
      if (element.image_id == image_id && element.feature_idx == feature_idx) {
        return true;
      }
    }
    return false;
