...
public: 
    ...
    */
    ...
    struct Node {
        ...
        inline void setIntrinsics(double fx, double fy, 
                                  double cx, double cy, 
                                  double alpha, 
                                  double k1, double k2) {
            camera_info.fx = fx;
        ...
        inline Eigen::Matrix3x4d getPose() const {
            return base::composePose(base::quaternion2RotationMatrix(camera_info.rotation), 
        ...
        inline Eigen::Matrix3d getCameraMatrix() const {
            Eigen::Matrix3d K; 
            K << camera_info.fx, camera_info.alpha, camera_info.cx, 
        ...
        inline bool hasTrack(const size_t &feature_idx, 
        ...
        inline Eigen::Matrix<uint8_t, 3, 1> getPoint2DColor(const size_t &idx) const {
            assert(idx < points_2D.size()); 
        ...
        /* The width of the corresponded image */
        int height{-1};
        /* List of 2D pixel coordinates (normalized by max(width, height)) of feature points observed in the image, */
        ...
        /* The observations of 3D points observed by this node. (track_id, feature_idx) */
    ...
    struct Edge {
        ...
        EIGEN_MAKE_ALIGNED_OPERATOR_NEW
        Edge(size_t _id, 
             const std::array<size_t, 2> &_node_ids, 
        ...
        /* The matching correspondences between 2D points of two nodes. [(point_idx_1, point_idx_2)] */
...
public: 
    ...
    /**
     * @brief Remove the node with ID id. This function will not remove the corresponded
     *  edges. If the node with the given ID does not exist, this function will do nothing.
     */
    ...
    */
    ...
    /**
     * @brief Remove the node with ID id and the corresponding edges. If the node with 
     * the given ID does not exist, this function will do nothing.
     */
    void removeNodeWithEdges(const size_t &id);

    /**
     * @brief Remove the edge with ID id. If the edge with the given ID does not exist, 
     * this function will do nothing. Also, this function will never remove any nodes.
     */
    void removeEdge(const size_t &id);

    /**
     * @brief Add an observation to the scene. Note that if there already exists an observation 
     * of track_id in node_id, this function will override the original observation
     * 
    */
    void addObservation(const size_t &track_id, 
                        const size_t &node_id, 
                        const size_t &feature_idx);

    /**
     * @brief Remove an observation from node_id. Note that this function will do nothing if the 
     * observation of track_id in node_id does not exist
    */
    void removeObservation(const size_t &track_id, 
                           const size_t &node_id);

    /**
     * @brief Add a track to the scene. Note that if there already exists an observation 
     * of track_id in node_id, this function will override the original observation
    */
    ...
    */
    ...
    /**
     * @brief Check whether the given track is visible in the given node (transitivity = 1)
     * 
     * @param track             the track to be checked
     * @param node_id           the id of node to be checked
     * @param point_2D_idx[out]  if the track is visible in node[node_id], it will be the index of the 
     *  track's observation in node[node_id]
    */
    bool checkTrackDirectVisibilityInNode(const base::Track &track, 
                                          const size_t &node_id, 
                                          size_t &point_2D_idx) const;

    Eigen::Matrix<uint8_t, 3, 1> computeTrackColor(const base::Track &track) const {
        int color[3] = {0, 0, 0}; 
        int num_elems = 0; 
        for (const auto &elem : track.getElements()) {
            ...
            const Eigen::Matrix<uint8_t, 3, 1> elem_color = getNodePoint2DColor(elem.image_id, elem.feature_idx); 
        ...
        return Eigen::Matrix<uint8_t, 3, 1>{static_cast<uint8_t>(util::clamp<int>(color[0] / num_elems, 0, 255)), 
                                            static_cast<uint8_t>(util::clamp<int>(color[1] / num_elems, 0, 255)), 
                                            static_cast<uint8_t>(util::clamp<int>(color[2] / num_elems, 0, 255))};
    }

    /**
     * @brief Find direct correspondences (i.e., transitivity = 1) of the given feature point 
     * 
     * @retval the correspondences represented by a list of (node_id, feature_idx) pairs 
    */
    std::vector<std::pair<size_t, size_t>> findDirectCorrespondences(const size_t &node_id, 
                                                                     const size_t &feature_idx) const; 
    
    /**
     * @brief Find transitive correspondences of the given feature point. Note that this function 
     * does not check ambiguity of correspondences, i.e., two correspondences with the same image 
     * id but different feature indices may exist in the result
     * 
     * @retval the correspondences represented by a list of (node_id, feature_idx) pairs 
    */
    std::vector<std::pair<size_t, size_t>> findTransitiveCorrespondences(const size_t &node_id, 
                                                                         const size_t &feature_idx, 
     ...
     * 
    */
    bool hasTrack(const size_t &node_id, 
                  const size_t &feature_idx, 
                  size_t *track_id) const;
     ...
     * 
     * @retval true if the node has bogus camera params or node_id does not exist, false otherwise 
    */
    ...
    /**
     * @brief Compute reprojection error of a 3D point in a node. This funciton will return -1 if the given node 
     * or feature point does not exist
    */
    double computeReprojectionError(const size_t &node_id, 
                                    const size_t &feature_idx, 
                                    const Eigen::Vector3d &point_3D) const; 
    
    inline std::unordered_map<size_t, Node*> getNodes() const {
        return nodes_; 
    }

    inline std::unordered_map<size_t, Edge*> getEdges() const {
        return edges_; 
    }
    ...
    */
    ...
    inline Eigen::Vector2d getNodePoint2DPosition(const size_t &node_id, 
                                          const size_t &point_2D_idx) const {
        assert(existsNode(node_id)); 
        return nodes_.at(node_id)->getPoint2DPosition(point_2D_idx); 
    }

    inline Eigen::Matrix<uint8_t, 3, 1> getNodePoint2DColor(const size_t &node_id, 
                                          const size_t &point_2D_idx) const {
        assert(existsNode(node_id)); 
