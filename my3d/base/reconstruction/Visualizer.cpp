...
void Visualizer::threadVisualization() {
    ...
    while (!window_.wasStopped()) {
        ...
        for (size_t i = 0; i < viz_data_.cameras.size(); ++i) {
            ...
            const cv::viz::WCameraPosition w_camera_postition{fov, 0.1, cv::viz::Color(viz_data_.cameras[i].color[0], 
...
void Visualizer::getVizData() {
    ...
    for (const size_t &camera_id : camera_ids) {
        ...
        const Eigen::Matrix3d rotation = base::quaternion2RotationMatrix(scene->getNodeRotation(camera_id)); 
