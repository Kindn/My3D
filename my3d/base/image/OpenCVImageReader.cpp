...
bool OpenCVImageReader::read(const std::string &path, 
                             Image &out, 
        ...
        if (!ifs.is_open()) {
            ...
            if (exif_info.Fields) {
                std::cout << "EXIF Information: \n"
                    ...
                    << "\tImage Resolution: " << cv_img.cols << "x" << cv_img.rows << " pixels\n"
                    << "\tCamera Model: " << exif_info.Make << " - " << exif_info.Model << "\n"
                    << "\tFocal Length: " << exif_info.FocalLength << " mm\n"
                    << "\tFocal Length in 35mm: " << exif_info.LensInfo.FocalLengthIn35mm << " mm" << std::endl;
            }
            ...
            int intrinsic_prior_src = extractIntrinsicsFromEXIF(exif_info, fx_prior, fy_prior, cx_prior, cy_prior, 
                                                                cv_img.cols, cv_img.rows);
            switch (intrinsic_prior_src)
            {
            case FocalLengthPriorType::FROM_CAMERA_DATABASE: 
                std::cout << "Obtained intrinsic prior from camera database: " << std::endl;
                break;
            case FocalLengthPriorType::FROM_EXIF_CALIB_INFO: 
                std::cout << "Obtained intrinsic prior from EXIF calibration information: " << std::endl;
                break;
            case FocalLengthPriorType::FROM_EXIF_FOCAL_LENGTH_35MM:
                std::cout << "Obtained intrinsic prior from EXIF focal length in 35mm: " << std::endl;
                break;
            case FocalLengthPriorType::FROM_DEFAULT_VALUE: 
                std::cout << "Obtained intrinsic prior from default values: " << std::endl;
                break;
            default:
                break;
            }
            ...
            camera_matrix_prior << fx_prior, 0, cx_prior, 0, fy_prior, cy_prior, 0, 0, 1;
            CameraBase *camera = new RadialPinHoleCamera(camera_matrix_prior);

            out.set(cv_img.rows, cv_img.cols, cv_img.channels(), cv_img.data, 0, path, camera);
        }
    }
...
bool OpenCVImageReader::save(const std::string &path, 
    ...
    cv::Mat cv_img(static_cast<int>(src.rows()), static_cast<int>(src.cols()), type);
