...
Image::Image(): 
data_{nullptr}, step0_{0}, step1_{0}, 
rows_{0}, cols_{0}, 
id_{0}, name_{""} {
...
step0_{cols * channels}, step1_{channels}, 
rows_{rows}, cols_{cols}, channels_(channels), 
id_{0}, name_{""}, camera_{nullptr} {
...
Image::Image(const size_t &rows, const size_t &cols, const size_t &channels, uint8_t *data, 
...
void Image::set(const size_t &rows, const size_t &cols, const size_t &channels, uint8_t *data, 
                const size_t &idx, const std::string &name, 
                CameraBase *camera) {
    if (rows == 0 || cols == 0 || channels == 0) {
        if (data_ != nullptr) {
            delete [] data_;
...
const uint8_t& Image::at(const size_t &i0, const size_t &i1, const size_t &i2) const {
    assert(i0 < rows_);
    assert(i1 < cols_);
    assert(i2 < channels_);
...
const uint8_t *Image::ptr(const size_t &i0, const size_t &i1, const size_t &i2) const {
    assert(i0 < rows_);
    assert(i1 < cols_);
    assert(i2 < channels_);
    return data_ + step0_ * i0 + step1_ * i1 + i2;
...
Eigen::Matrix<uint8_t, Eigen::Dynamic, 1> Image::getChannel(const size_t &row, const size_t &col) const {
    assert(row <= rows_);
    assert(col <= cols_);
...
Eigen::Vector2d Image::getGrandient(const size_t &x, const size_t &y, const size_t &channel) const {
    assert(!isEmpty());
    assert(x >= 0 && x < cols_);
    assert(y >= 0 && y < rows_); 
