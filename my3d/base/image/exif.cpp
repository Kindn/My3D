...
int extractIntrinsicsFromEXIF(const TinyEXIF::EXIFInfo &exif_info, 
    ...
    if (exif_focal_length > 0.0 && !exif_camera_model.empty()) {
        ...
        CameraInfo const *camera_info = db->lookup(exif_camera_maker, exif_camera_model);
    ...
    if (exif_focal_length > 0.0 && sensor_width_mm > 0.0 && sensor_height_mm > 0.0) {
    ...
    if (exif_calib.FocalLength > 0.0 && 
        exif_calib.OpticalCenterX > 0.0 && 
