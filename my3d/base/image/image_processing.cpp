...
void blurGaussian(const Image &src, Image &dst, const double &sigma) {
    ...
    for (int i = -half_size_kernal; i <= half_size_kernal; ++i) {
        kernal(i + half_size_kernal) = 
            std::exp(-0.5 * (i / sigma) * (i / sigma));// * (1.0 + (i + half_size_kernal) / 1000.0); // util::computeGaussian(i, sigma, 0.0); 
    }
    kernal /= kernal.sum();
...
void blurGaussian(const Image &src, EigenVec<Eigen::MatrixXd> &dst, const double &sigma) {
    assert(sigma > 0); 
    ...
    for (int i = -half_size_kernal; i <= half_size_kernal; ++i) {
        kernal(i + half_size_kernal) = 
            std::exp(-0.5 * (i / sigma) * (i / sigma));// * (1.0 + (i + half_size_kernal) / 1000.0);
...
void blurGaussian(const Eigen::MatrixXd &src, Eigen::MatrixXd &dst, const double &sigma) {
    assert(sigma > 0); 
    ...
    /* Compute Gaussian Kernal */
    const int half_size_kernal = 3.0 * sigma > 1.0 ? (int)std::ceil(3.0 * sigma) : 1; 
    Eigen::VectorXd kernal(2 * half_size_kernal + 1);
    for (int i = -half_size_kernal; i <= half_size_kernal; ++i) {
        kernal(i + half_size_kernal) = 
            std::exp(-0.5 * (i / sigma) * (i / sigma));// * (1.0 + (i + half_size_kernal) / 1000.0); 
    ...
    for (size_t row = 0; row < rows; ++row) {
        for (size_t col = 0; col < cols; ++col) {
            for (int idx = -half_size_kernal; idx <= half_size_kernal; ++idx) {
                const size_t c = util::clamp<size_t>(col + idx, 0, cols - 1); 
                tmp_dst(row, col) += kernal(half_size_kernal + idx) * 
                        static_cast<double>(src(row, c)); 
            }
        }
    }
    ...
    for (size_t row = 0; row < rows; ++row) {
        for (size_t col = 0; col < cols; ++col) {
            for (int idx = -half_size_kernal; idx <= half_size_kernal; ++idx) {
                const size_t r = util::clamp<size_t>(row + idx, 0, rows - 1); 
                dst(row, col) += kernal(half_size_kernal + idx) * 
                        tmp_dst(r, col); 
            }
        }
    }
    ...
    for (size_t row = 0; row < scaled_rows; ++row) {
        ...
        for (size_t col = 0; col < scaled_cols; ++col) {
            ...
            for (size_t channel = 0; channel < channels; ++channel) {
                const double x11 = static_cast<double>(src.at(row_src1, col_src1, channel)); 
                const double x12 = static_cast<double>(src.at(row_src1, col_src2, channel)); 
                const double x21 = static_cast<double>(src.at(row_src2, col_src1, channel)); 
                const double x22 = static_cast<double>(src.at(row_src2, col_src2, channel)); 
                tmp_dst.at(row, col, channel) = 
                    static_cast<uint8_t>(util::clamp<int>(util::computeBilinearInterpolation(x11, x12, x21, x22, 
                                                                            col_src1, col_src2, row_src1, row_src2, 
                                                                            col_src, row_src), 0, 255)); 
            }
        }
    }
...
computeChannelWiseImageDifference(const Image &image1, 
                                  const Image &image2) {
    ...
    for (size_t channel = 0; channel < channels; ++channel) {
        ...
        for (size_t row = 0; row < rows; ++row) {
            for (size_t col = 0; col < cols; ++col) {
                diff(row, col) = 
                    static_cast<double>(image1.at(row, col, channel)) - 
...
void convertImageColorBGR2GrayScale(const Image &src, Image &dst) {
    ...
    for (size_t row = 0; row < rows; ++row) {
        for (size_t col = 0; col < cols; ++col) {
            ...
            //     static_cast<uint8_t>(0.114 * bgr(0) + 0.587 * bgr(1) + 0.299 * bgr(2)); 
            double gray = (double(bgr(0)) + double(bgr(1)) + double(bgr(2))) / 3.0 + 0.5; 
...
void drawCircle(const base::Image &src, base::Image &dst, 
    ...
    if (radius <= std::numeric_limits<double>::epsilon()) {
        if (static_cast<int>(center_x) >= 0 && static_cast<int>(center_x) <= static_cast<int>(cols) - 1 && 
            static_cast<int>(center_y) >= 0 && static_cast<int>(center_y) <= static_cast<int>(rows) - 1) {
            if (channels == 1) {
                dst.at(center_y, center_x, 0) = 
                    static_cast<uint8_t>(0.114 * color[0] + 0.587 * color[1] + 0.299 * color[2]); 
            } else if (channels == 3) {
                for (int i = 0; i < 3; ++i) {
    ...
    for (double angle = 0.0; angle < 2.0 * M_PI; angle += angle_step) {
        for (double dr = -thickness * 0.5; dr <= thickness * 0.5; ++dr) { 
            const int x = static_cast<int>(center_x + (scaled_radius + dr) * std::cos(angle)); 
            const int y = static_cast<int>(center_y + (scaled_radius + dr) * std::sin(angle)); 
            if (x >= 0 && x <= static_cast<int>(cols) - 1 && 
                y >= 0 && y <= static_cast<int>(rows) - 1) { 
                if (channels == 1) {
                    dst.at(y, x, 0) = 
                        static_cast<uint8_t>(0.114 * color[0] + 0.587 * color[1] + 0.299 * color[2]); 
                } else if (channels == 3) {
...
void drawLine(const base::Image &src, base::Image &dst, 
    ...
    if (x1 >= 0 && x1 <= static_cast<int>(cols) - 1 && 
        y1 >= 0 && y1 <= static_cast<int>(rows) - 1) {
        if (channels == 1) {
            dst.at(y1, x1, 0) = 
                    static_cast<uint8_t>(0.114 * color[0] + 0.587 * color[1] + 0.299 * color[2]); 
        } else if (channels == 3) {
    ...
    if (x2 >= 0 && x2 <= static_cast<int>(cols) - 1 && 
        y2 >= 0 && y2 <= static_cast<int>(rows) - 1) {
        if (channels == 1) {
            dst.at(y2, x2, 0) = 
                    static_cast<uint8_t>(0.114 * color[0] + 0.587 * color[1] + 0.299 * color[2]); 
        } else if (channels == 3) {
    ...
    // std::cout << point1.transpose() << " " << point2.transpose() << direction.norm() << " " << (point2 - point1).norm() << std::endl; 
