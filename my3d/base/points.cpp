...
void normalize2DPointSet(const EigenVec<Eigen::Vector2d> &src, 
    ...
    std::vector<Eigen::Vector2d, Eigen::aligned_allocator<Eigen::Vector2d>> tmp_src = src;
    dst.clear();
...
void normalize2DPointSet(const EigenVec<Eigen::Vector2d> &src, 
    ...
    t_mat.setIdentity(); t_mat.block<2, 1>(0, 2) = t;
    scale_mat.setIdentity(); scale_mat.block<2, 2>(0, 0) *= scale;
