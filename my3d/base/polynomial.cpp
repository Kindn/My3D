...
bool solvePolynomialCompanionMatrix(const Eigen::VectorXd &coeffs,
  ...
  if (degree <= 0) {
    std::cout << __PRETTY_FUNCTION__ << "Error: Degree is not positive. "
              << std::endl;
    return false;
  } else if (degree == 1) {
    std::cout << __PRETTY_FUNCTION__
              << "Waring: Directly solving linear polynomial. " << std::endl;
    return solveLinearPolynomial(effective_coeffs, roots_real, roots_imag);
  } else if (degree == 2) {
    std::cout << __PRETTY_FUNCTION__
              << "Waring: Directly solving quadratic polynomial. " << std::endl;
  ...
  C.rightCols<1>() = -effective_coeffs.head(effective_degree) /
                     effective_coeffs(effective_degree);
  /* Solve for the eigenvalues of companion matrix */
  Eigen::EigenSolver<Eigen::MatrixXd> solver(C, false);
  if (solver.info() != Eigen::Success) {
    // std::cout << __PRETTY_FUNCTION__ << "Warning: EigenSolver failed" <<
    // std::endl;
    return false;
  }
...
bool refineRoots(const Eigen::VectorXd &coeffs, Eigen::VectorXd &real_roots,
  ...
  for (uint32_t iter = 0; iter < iter_num; ++iter) {
    for (Eigen::Index root_idx = 0; root_idx < (long)real_roots.size();
         ++root_idx) {
      double val = base::computePolynomial(coeffs, real_roots(root_idx));
      double deriv =
          base::computePolynomialDerivative(coeffs, real_roots(root_idx), 1);
