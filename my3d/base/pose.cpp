...
Eigen::Matrix3d
quaternion2RotationMatrix(const Eigen::Quaterniond &quaternion) {
...
bool checkCheirality(const Eigen::Matrix3d &rotation,
  ...
  for (size_t i = 0; i < num_pnts; ++i) {
    ...
    // std::cout << point_3D.z() << " " << (rotation * point_3D +
    // translation).z() << std::endl;
    const double depth1 = point_3D.z();
    const double depth2 = (rotation * point_3D + translation).z();
    if (depth1 > kMinDepth && depth1 < max_depth && depth2 > kMinDepth &&
        depth2 < max_depth) {
      ret = true;
      if (points_3D) {
