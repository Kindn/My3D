...
RadialPinHoleCamera::RadialPinHoleCamera(const Eigen::Matrix3d &K, double k1,
                                         double k2)
    : fx_(K(0, 0)), fy_(K(1, 1)), cx_(K(0, 2)), cy_(K(1, 2)), alpha_(K(0, 1)),
      k1_(k1), k2_(k2) {
  assert(fx_ > 0 && fy_ > 0 && cx_ >= 0 && cy_ >= 0);
}

Eigen::Vector2d
RadialPinHoleCamera::project(const Eigen::Vector3d &point3d) const {
...
Eigen::Vector2d
RadialPinHoleCamera::pix2Norm(const Eigen::Vector2d &pix) const {
...
Eigen::Matrix3d RadialPinHoleCamera::getCameraMatrix() const {
  Eigen::Matrix3d K;
  K << fx_, alpha_, cx_, 0, fy_, cy_, 0, 0, 1;

  return K;
}

Eigen::Vector2d RadialPinHoleCamera::pixel2NormalizedIgnoringDistortion(
    const Eigen::Vector2d &pix) const {
  return Eigen::Vector2d((pix.x() - cx_) / fx_, (pix.y() - cy_) / fy_);
}

Eigen::Vector2d RadialPinHoleCamera::undistort(
    const Eigen::Vector2d &distorted_point_2D) const {
