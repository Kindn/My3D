...
Eigen::Vector2d computeReprojectionResidual(const Eigen::Vector3d &point_3D, 
    ...
    const Eigen::Vector2d proj = (projection_matrix * point_3D.homogeneous()).hnormalized(); 
    return proj - point_2D;
}
...
double computeReprojectionError(const Eigen::Vector3d &point_3D, 
                                ...
                                const Eigen::Matrix3x4d &proj_matrix,  
                                const base::CameraBase *camera) {
    return computeReprojectionResidual(point_3D, point_2D, proj_matrix, camera).norm();
}

double computeReprojectionError(const Eigen::Vector3d &point_3D, 
                                const Eigen::Vector2d &point_2D, 
                                const Eigen::Matrix3x4d &projection_matrix) {
    return computeReprojectionResidual(point_3D, point_2D, projection_matrix).norm();
}
