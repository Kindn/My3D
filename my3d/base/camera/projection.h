...
/**
 * @brief Compute the projection matrix corresponding to the given camera intrinsics and relative pose [R t]
*/
Eigen::Matrix3x4d computeProjectionMatrix(double fx, double fy, double cx, double cy, double alpha, 
...
*/
Eigen::Vector2d computeReprojectionResidual(const Eigen::Vector3d &point_3D, 
                                            const Eigen::Vector2d &point_2D, 
                                            const Eigen::Matrix3x4d &proj_matrix,  
                                            const base::CameraBase *camera); 

/**
 * @brief Compute reprojection residual of a point. This function only suits for cameras whose projection 
 * could be represented as a 3x4 matrix
 * 
 ...
 * 
 * @param point_3D     point's coordinate in world frame s.t. pixel = camera->projection(point_3D)
 * @param point_2D     pixel coordinate of point_3D
 * @param proj_matrix  camera extrinsics [R t]
 * @param camera       the camera model
*/
...
/**
 * @brief Compute reprojection error of a point. This function only suits for cameras whose projection 
 * could be represented as a 3x4 matrix
 * 
 * @param point_3D     point's coordinate in camera frame s.t. pixel = camera->projection(point_3D)
 * @param point_2D     pixel coordinate of point_3D
 * @param projection_matrix  the camera model
*/
double computeReprojectionError(const Eigen::Vector3d &point_3D, 
 ...
 * 
 * @param point_2D     point's coordinate in normalized camera frame s.t. pixel = camera->projection(point_3D)
