...
public:
  enum NextBestViewSelectionMethod { MAX_VISIBLE_TRACK_NUM = 0 };
  ...
  struct Config {
    ...
    /* Only the observations with reprojections not greater than
       max_track_reproj_error (in pixels) should be included in a track */
    ...
    /* Minimum inlier ratio of absolute pose estimation in next image
     * registration */
    double min_abs_pose_inlier_ratio{0.2};
    /* Minimum inlier number of absolute pose estimation in next image
     * registration */
    size_t min_abs_pose_inlier_num{15};
    /* Epsilon for AP3P */
    double abs_pose_epsilon{1e-5};
    /* Method for next best view selection */
    NextBestViewSelectionMethod nbv_method{
        NextBestViewSelectionMethod::MAX_VISIBLE_TRACK_NUM};
  ...
  struct BundleAdjustmentReport {
    ...
    void print() const {
      ...
      std::cout << "number of merged observations: " << num_merged_observations
                << std::endl;
      std::cout << "number of completed observations: "
                << num_completed_observations << std::endl;
    }
  };

  // typedef estimator::RANSAC<estimator::AP3PPoseEstimator>
...
public:
  ...
  /**
   * @brief Compute initial tracks (without triangulation) according to the
   * scene graph
   */
   ...
   */
  Report
  reconstruct(const std::shared_ptr<base::Reconstruction> reconstruction);

  /**
   * @brief Find the initial image pair. Should be called after
   * beginReconstruction()
   *
   * @param inital_pair[out]     the image_ids/node_ids of the initial image
   * pair
  ...
  /**
   * @brief Find candidates initial image pair. Should be called after
   * beginReconstruction()
   *
   * @param inital_pairs[out]     the image_ids/node_ids of the initial image
   * pair
   * @param initial_edge_ids[out] the edge_id of the initial image pair
   */
  bool findInitialImagePairCandidates(
   ...
   */
  bool findNextImagesWithMaxVisibleTracks(
  ...
  /**
   * @brief Global bundle adjustment for all registered images and existing
   * tracks
   */
  BundleAdjustmentReport
  solveGlobalBundleAdjustment(const bool &should_update = false);

  /**
   * @brief Local bundle adjustment for the last registered image, its visible
   * tracks, and covisible images
   */
  BundleAdjustmentReport
  solveLocalBundleAdjustment(const size_t &image_id,
                             const bool &should_update = false);

  /**
   * @brief Update parameters of cameras and coordinates of tracks according to
   * the result of BA
   */
  ...
  /**
   * @brief Try to create new tracks from the given image. Note that the image
   * should be registered and its pose should be known
   *
  ...
  /**
   * @brief Triangulte a given track. The given track may be splitted into
   * several new tracks according to the triangulation inliers
   *
  ...
  /**
   * @brief Robustly triangulate a list of observations (represented by track
   * elements). Please make sure that there is not an observation's
   * correspongding camera has bogus parameters or undetermined pose
   */
  ...
  /**
   * @brief Try to recursively add observations that may have failed to be
   * triangulated before due to inaccurate poses to a track
   *
  ...
  /**
   * @brief Complete triangulations for image. Tries to create new tracks for
   * not yet triangulated observations and tries to complete existing tracks.
   ...
   */
  size_t
  filterTracksWithLargeReprojectionError(const double &max_reproj_error,
   ...
   */
  size_t
  filterTracksWithSmallTriangulationAngle(const double &min_tri_angle_rad,
                                          const std::vector<size_t> &track_ids,
                                          const bool &init = false);
...
protected:
  ...
  // base::Scene *scene_;
  // /* (track_id, track). Tracks are 3D points that are already been
  // reconstructed */ std::unordered_map<size_t, base::Track*> tracks_;
