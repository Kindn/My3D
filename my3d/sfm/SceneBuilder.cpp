...
bool SceneBuilder::build(const std::vector<base::Image> &images, 
    ...
    for (size_t i = 0; i < num_images; ++i) {
        ...
        for (size_t j = 0; j < key_points.size(); ++j) {
            ...
            const Eigen::Matrix<uint8_t, 3, 1> color_bgr = images[i].getChannel(static_cast<size_t>(position.y()), 
                                                                            static_cast<size_t>(position.x())); 
            points_2D.emplace_back(position, Eigen::Matrix<uint8_t, 3, 1>(color_bgr.z(), color_bgr.y(), color_bgr.x()));

        }
        base::Scene::CameraInfo camera_info; 
        const auto camera_model = std::dynamic_pointer_cast<base::RadialPinHoleCamera>(images[i].getCameraModel());
        ...
        bool success = scene.addNode(i, images[i].cols(), images[i].rows(), points_2D, images[i].getName(), camera_info);
        if (!success) {
            return false;
        }
    }
    ...
    for (auto &matching : pair_wise_matchings) {
        ...
        const EigenVec<Eigen::Vector2d> &key_points1 = key_points_list[matching.query_image_idx];
        const EigenVec<Eigen::Vector2d> &key_points2 = key_points_list[matching.train_image_idx];
        ...
        ImagePairVerificationReport veri_report = verifyImagePair(image1, image2, key_points1, key_points2, matching, model_type, inliers, &two_view_geom_info);
        if (veri_report.success && 
            ...
            for (auto &inlier : inliers.matches) {
                correspondences.push_back(std::make_pair(inlier.query_index, inlier.train_index));
            }
            bool success = scene.addEdge(edge_id, 
            ...
            
            {
                ...
                std::cout << "[SceneBuilder] Added edge " << edge_id << std::endl
                          << "\timage 1: " << image1.getName() << "; id = " << matching.query_image_idx << std::endl 
                          << "\timage 2: " << image2.getName() << "; id = " << matching.train_image_idx << std::endl 
                          << "\tedge type: " << edge_type_str << std::endl 
                          << "\tmatch inlier ratio: " << veri_report.match_inlier_ratio << std::endl 
                          ...
                          << "\tH inlier ratio: " << veri_report.homography_inilier_ratio << std::endl 
                          ...
                          << "\ttriangulated " << two_view_geom_info.triangulated_points.size() << " points " << std::endl 
                          ...
                          << "\t\tinlier ratio of fundamental matrix estimation: " << veri_report.inlier_ratio_f << std::endl 
                          << "\t\tinlier ratio of homography estimation: " << veri_report.inlier_ratio_h << std::endl 
                          << "\t\tinlier ratio of essential matrix estimation: " << veri_report.inlier_ratio_e << std::endl; 
            }
...
SceneBuilder::ImagePairVerificationReport
SceneBuilder::verifyImagePair(const base::Image &image1, 
                              const base::Image &image2, 
    ...
    if (num_matches < config_.min_num_inliers || 
        num_matches < estimator::FundamentalMatrixSevenPointEstimator::kMinNumSamples || 
        num_matches < estimator::FundamentalMatrixEightPointEstimator::kMinNumSamples || 
        num_matches < estimator::HomographyEstimator::kMinNumSamples || 
        num_matches < estimator::EssentialMatrixEightPointEstimator::kMinNumSamples) {
    ...
    typedef estimator::LORANSAC<estimator::FundamentalMatrixSevenPointEstimator, 
                                estimator::FundamentalMatrixEightPointEstimator> FundamentalEstimator;
    typedef estimator::LORANSAC<estimator::HomographyEstimator, 
                                estimator::HomographyEstimator> HomographyEstimator;
    typedef estimator::LORANSAC<estimator::EssentialMatrixFivePointEstimator, 
                                estimator::EssentialMatrixFivePointEstimator> EssentialEstimator;
    // const size_t kf = estimator::FundamentalMatrixEightPointEstimator::kMinNumSamples; 
    // const size_t kh = estimator::HomographyEstimator::kMinNumSamples; 
    // const size_t ke = estimator::EssentialMatrixEightPointEstimator::kMinNumSamples; 
    ...
    const feature::Matches inlier_matches = 
        feature::getInlierMatches(key_points1, key_points2, close_matches, config_.ransac_config); 
    ...
    veri_report.match_inlier_ratio = 
        static_cast<double>(inlier_matches.size()) / static_cast<double>(num_matches); 
    ...
    inliers_fundamental.matches.clear();
    {
        ...
        veri_report.inlier_ratio_f = 
            static_cast<double>(inliers_fundamental.matches.size()) / static_cast<double>(inlier_matches.size()); 
    ...
    inliers_homography.matches.clear();
    {
        ...
        // h_mat_ransac_config.max_iter_num = std::max(2 * util::computeCombinationNumber(points1.size(), kh), 
        ...
        veri_report.inlier_ratio_h = 
            static_cast<double>(inliers_homography.matches.size()) / static_cast<double>(inlier_matches.size()); 
        if (two_view_geom_info) {
    ...
    const double epsilon_HF = static_cast<double>(inliers_homography.matches.size()) / 
                              static_cast<double>(inliers_fundamental.matches.size());
    const double homography_inlier_ratio = static_cast<double>(inliers_homography.matches.size()) / 
    ...
    //* Compute the essential matrix using the calibration information and determine 
    //* whether the image pair is well calibrated
    const std::shared_ptr<base::RadialPinHoleCamera> camera_model1 = 
        std::dynamic_pointer_cast<base::RadialPinHoleCamera>(image1.getCameraModel());
    const std::shared_ptr<base::RadialPinHoleCamera> camera_model2 = 
        std::dynamic_pointer_cast<base::RadialPinHoleCamera>(image2.getCameraModel());
    ...
    EigenVec<Eigen::Vector2d> inlier_points_2DC1, inlier_points_2DC2;
    {
        for (size_t i = 0; i < points1.size(); ++i) {
            points_2DC1.push_back(camera_model1->pixel2NormalizedIgnoringDistortion(points1[i]));
            points_2DC2.push_back(camera_model2->pixel2NormalizedIgnoringDistortion(points2[i]));
        }
        estimator::RANSACConfig e_mat_ransac_config = config_.ransac_config; 
        e_mat_ransac_config.max_inlier_error = 
            0.5 * (camera_model1->thresholdPix2Norm(config_.ransac_config.max_inlier_error) + 
                   camera_model2->thresholdPix2Norm(config_.ransac_config.max_inlier_error)); 
        ...
        veri_report.inlier_ratio_e = 
            static_cast<double>(inliers_essential.matches.size()) / static_cast<double>(inlier_matches.size()); 
        if (two_view_geom_info) {
            two_view_geom_info->essential_matrix = essential_matrix;
        }
    }

    const double epsilon_HE = static_cast<double>(inliers_homography.matches.size()) / 
                              static_cast<double>(inliers_essential.matches.size());
    veri_report.epsilon_HE = epsilon_HE; 


    const double epsilon_EF = static_cast<double>(inliers_essential.matches.size()) / 
                                static_cast<double>(inliers_fundamental.matches.size());
    veri_report.epsilon_EF = epsilon_EF; 
    
    if (epsilon_EF >= config_.min_epsilon_EF && 
        inliers_essential.matches.size() >= config_.min_num_inliers) { //*  This indicates that the image pair is well calibrated
        ...
        if (inliers_essential.matches.size() >= inliers_fundamental.matches.size()) {
        ...
        if (epsilon_HE > config_.max_H_inlier_ratio) {
            ...
            model_type = base::Scene::Edge::SceneEdgeModelType::GENERAL_AND_CALIBRATED; 
        }
        ...
        if (epsilon_HF > config_.max_H_inlier_ratio) {
            ...
            model_type = base::Scene::Edge::SceneEdgeModelType::GENERAL_AND_UNCALIBRATED; 
        }
    ...
    if (!estimateRelativePose(inlier_points_2DC1, inlier_points_2DC2, essential_matrix, 
                              inlier_h_points1, inlier_h_points2, homography, 
                              K1, K2, 
                              model_type, 
                              R, t, 
                              points_3D, alphas, median_alpha)) {
        model_type = base::Scene::Edge::SceneEdgeModelType::INVALID; 
        veri_report.success = true; 
        return veri_report; 
    }
...
bool SceneBuilder::estimateRelativePose(const EigenVec<Eigen::Vector2d> &essential_inliers1, 
                                        ...
                                        EigenVec<Eigen::Vector3d> &tri_points, 
                                        std::vector<double> &tri_angles, 
                                        double &median_tri_angle) {
    tri_points.clear(); 
    tri_angles.clear(); 
    if (model_type == base::Scene::Edge::SceneEdgeModelType::GENERAL_AND_CALIBRATED || 
        model_type == base::Scene::Edge::SceneEdgeModelType::GENERAL_AND_UNCALIBRATED) {
        estimator::poseFromEssentialMatrix(E, essential_inliers1, essential_inliers2, R, t, tri_points);
    } else if (model_type == base::Scene::Edge::SceneEdgeModelType::PANORAMIC_OR_PLANNAR || 
               model_type == base::Scene::Edge::SceneEdgeModelType::PANORAMIC || 
               model_type == base::Scene::Edge::SceneEdgeModelType::PLANNAR) {
        Eigen::Vector3d normal; 
        estimator::poseFromHomography(H, 
                                      K1, K2, 
                                      homography_inliers1, homography_inliers2, 
                                      R, t, normal, tri_points);
    } else {
        return false; 
    }
    ...
    for (size_t i = 0; i < tri_points.size(); ++i) {
        tri_angles[i] = base::computeTriangulationAngle(Eigen::Vector3d::Zero(), -R.transpose() * t, tri_points[i]);
    }
    std::sort(tri_angles.begin(), tri_angles.end());
                
    if (tri_angles.size() % 2 == 0) { 
        median_tri_angle = 0.5 * (tri_angles[tri_angles.size() / 2 - 1] + tri_angles[tri_angles.size() / 2]);
    } else { 
        median_tri_angle = tri_angles[tri_angles.size() / 2];
    } 

    if (model_type == base::Scene::Edge::SceneEdgeModelType::PANORAMIC_OR_PLANNAR) {
        if (t.norm() == 0) {
            model_type = base::Scene::Edge::SceneEdgeModelType::PANORAMIC; 
        } else {
            model_type = base::Scene::Edge::SceneEdgeModelType::PLANNAR; 
