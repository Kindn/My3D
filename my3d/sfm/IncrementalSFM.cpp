...
IncrementalSFM::IncrementalSFM(const Config &config): 
config_{config} {
...
double IncrementalSFM::computeInitialPairScore(const EdgeInfo &edge_info) const {
    const base::Scene * const scene = reconstruction_->getScene(); 
    const double score = 
        0.3 * static_cast<double>(edge_info.num_connected_nodes_1) / (static_cast<double>(scene->getNumNodes()) - 1) + 
        0.3 * static_cast<double>(edge_info.num_connected_nodes_2) / (static_cast<double>(scene->getNumNodes()) - 1) + 
        0.4 * edge_info.median_triangulated_angle / M_PI; 
    
    return score; 
}

void IncrementalSFM::config(const Config &config) {
    config_ = config;
}

void IncrementalSFM::beginReconstruction(const std::shared_ptr<base::Reconstruction> reconstruction) {
...
void IncrementalSFM::endReconstruction(Report &report) {
    ...
    for (const auto &image_id : reconstruction_->getRegisteredImageIDs()) {
        report.camera_infos.insert(std::make_pair(image_id, 
            reconstruction_->getScene()->getNode(image_id)->camera_info)); 
    }
...
void IncrementalSFM::reset() {
    reconstruction_->reset(); 
}

IncrementalSFM::Report 
IncrementalSFM::reconstruct(const std::shared_ptr<base::Reconstruction> reconstruction) {
    ...
    if (!findInitialImagePairCandidates(initial_image_pairs, initial_edge_ids)) { 
        std::cout << "[ERROR] Failed to find initial image pair. Reconstruction aborted. " << std::endl;
        report.success = false;
        return report; 
    } else {
        std::cout << "[INFO] Found " << initial_image_pairs.size() << " initial image pair candidates. " 
        ...
        for (size_t idx = 0; idx < initial_image_pairs.size(); ++idx) {
            ...
            // std::cout << "Trying to register edge " << initial_edge_id << " as initial pair. " << std::endl; 
            reset(); 
            if (registerInitialImagePair(initial_image_pair, initial_edge_id)) {
                ...
                std::cout << "[INFO] Model type: " << scene->getEdge(initial_edge_id)->model_type << std::endl; 
                std::cout << "[INFO] Median triangulation angle: " 
                          << util::rad2Deg(scene->getEdge(initial_edge_id)->two_view_geom_info.median_triangulation_angle) << " deg(s) " << std::endl;
                std::cout << "[INFO] Initial image 0 pose: [" << scene->getNodeTranslation(initial_image_pair.first).transpose() << "]"
                            " " << scene->getNodeRotation(initial_image_pair.first).coeffs().transpose() << "] " << std::endl; 
                std::cout << "[INFO] Initial image 1 pose: [" << scene->getNodeTranslation(initial_image_pair.second).transpose() << 
                        " " << scene->getNodeRotation(initial_image_pair.second).coeffs().transpose() << "] " << std::endl; 

                BundleAdjustmentReport gba_report = 
                    solveGlobalBundleAdjustment(false); 
                // gba_report.print(); 
                if (gba_report.success) {
                    ...
                    std::cout << "[INFO] Filtered " << num_filtered_observations << " observations " << std::endl; 
                    // const size_t num_filtered_images = filterAllImages(); 
                    // std::cout << "[INFO] Filtered " << num_filtered_images << " images " << std::endl; 
                    if (reconstruction_->getRegisteredImageIDs().size() == 0 || 
                        reconstruction_->getTracks().size() < config_.min_num_points) {
                        std::cout << "[ERROR] Initial image registration failed. Trying anothor image pair. " << std::endl; 
            ...
            if (success_registration) {
                ...
                std::cout << "[WARNING] Failed to resgister inital pair (" 
                          << initial_image_pair.first << ", " << initial_image_pair.second << "), " 
        ...
        if (!success_registration) {
            std::cout << "[ERROR] Failed to register initial image pair. Reconstruction aborted. " << std::endl; 
    ...
    while (reconstruction_->getNumRegisteredImages() + 
           reconstruction_->getNumFilteredImages() < reconstruction_->getScene()->getNumNodes()) {
        std::cout << "================================" << std::endl; 
        if (!findNextImageCandidates(next_image_ids, corrs_3D2Ds)) {
            std::cout << "[WARNING] Failed to find next image " << reconstruction_->getRegisteredImageIDs().size() << 
            ...
            for (size_t i = 0; i < next_image_ids.size(); ++i) { 
                ...
                const std::unordered_map<size_t, size_t> correspondences_3D2D = corrs_3D2Ds[i]; 
                std::cout << "================================" << std::endl; 
                std::cout << "[INFO] Trying to register image " << next_image_id << "(" << reconstruction->getScene()->getNode(next_image_id)->name << 
                            "); Image sees " << correspondences_3D2D.size() << "/" << reconstruction_->getTracks().size() << " 3D points. " << std::endl; 
                ...
                if (!registerNextImage(next_image_id, correspondences_3D2D)) {
                    std::cout << "[WARNING] Failed to register next image " << next_image_id << std::endl; 
                    reconstruction_->deregisterImage(next_image_id); 
                    continue; 
                } else {
                    std::cout << "[INFO] Registered next image (id = " << next_image_id << ", " << 
                    reconstruction->getScene()->getNode(next_image_id)->name << std::endl; 

                    BundleAdjustmentReport ba_report; 
                    // gba_report = solveGlobalBundleAdjustment(true); 
                    // if (gba_report.success) {
                    //     report.reprojection_rmse = gba_report.final_squared_reprojection_error; 
                    //     std::cout << "[INFO] Pre Global BA succeeded. " << std::endl;
                    // } else {
                    //     std::cout << "[INFO] Pre Global BA failed. Reconstruction aborted " << std::endl; 
                    // }
                    ...
                    num_filtered = filterObservationsWithNegativeDepth();
                        std::cout << "[INFO] Filtered " << num_filtered << " observations with negative depths before BA. " << std::endl; 

                    const auto registered_image_ids = reconstruction_->getRegisteredImageIDs(); 
                    const auto filtered_image_ids = reconstruction_->getFilteredImageIDs(); 
                    const auto tracks = reconstruction_->getTracks(); 

                    ba_report = solveLocalBundleAdjustment(next_image_id, false); 
                    if (ba_report.success) {
                        report.reprojection_rmse = ba_report.final_squared_reprojection_error; 
                        ...
                        std::cout << "[INFO] Filtered " << num_filtered << " observations " << std::endl; 
                        std::cout << "[INFO] Completed " << num_completed << " observations " << std::endl; 
                        std::cout << "[INFO] Merged " << num_merged << " observations " << std::endl; 
                        // size_t num_triangulated = 0;
                        // num_triangulated += triangulateImage(next_image_id);
                        std::cout << "[INFO] Triangulated " << num_triangulated << " observations " << std::endl; 
                        std::cout << "\n\tRegistration for image {" << next_image_id << "} done. " << std::endl; 
                        std::cout << "\tnum_points_3D: " << tracks.size() << std::endl; 
                        std::cout << "\tnum_registered_images: " << registered_image_ids.size() << std::endl; 
                        std::cout << "\tnum_filtered_images: " << filtered_image_ids.size() << std::endl; 
                        std::cout << "\tnum_remaining_images: " << reconstruction_->getScene()->getNumNodes() - registered_image_ids.size() - filtered_image_ids.size() << std::endl;
                        std::cout << "================================\n" << std::endl; 
                        success = true; 
                        // const size_t num_filtered_images = filterAllImages(); 
                        // std::cout << "[INFO] Filtered " << num_filtered_images << " images " << std::endl; 
                        ...
                        success = false; 
                        std::cout << "[INFO] Local BA failed when register image " << next_image_id << std::endl; 
                        reconstruction_->deregisterImage(next_image_id); 
                        // break;
                    }
                    ...
                    if (step % kGlobalBAInterval == 0) {
                        ...
                        if (ba_report.success) {
                            ...
                            std::cout << "\tnum_points_3D: " << tracks.size() << std::endl; 
                            std::cout << "\tnum_registered_images: " << registered_image_ids.size() << std::endl; 
                            std::cout << "\tnum_filtered_images: " << reconstruction_->getFilteredImageIDs().size() << std::endl; 
                            std::cout << "\tnum_remaining_images: " << 
                            reconstruction_->getScene()->getNumNodes() - registered_image_ids.size() - filtered_image_ids.size() << std::endl;
                            std::cout << "================================\n" << std::endl; 
                            success = true; 
                            // const size_t num_filtered_images = filterAllImages(); 
                            // std::cout << "[INFO] Filtered " << num_filtered_images << " images " << std::endl; 
                            ...
                            success = false; 
                            std::cout << "[INFO] Global BA failed when register image " << next_image_id << std::endl; 
                            reconstruction_->deregisterImage(next_image_id); 
                            // break;
                        }
                    } else {
    ...
    if (true) {
        std::cout << "[INFO] Doing final global bundle adjustment ... " << std::endl; 
        reconstruction_->clearModifiedTrackIDs(); 
        reconstruction_->clearNewTrackIDs(); 
        reconstruction_->clearNewImageIDs(); 
        BundleAdjustmentReport gba_report = 
        ...
        if (gba_report.success) {
            ...
            std::cout << "[INFO] Final global BA failed. Reconstruction aborted. " << std::endl; 
            report.success = false; 
        }
    }
...
bool IncrementalSFM::findInitialImagePair(std::pair<size_t, size_t> &initial_pair, 
                                          size_t &initial_edge_id) {
...
bool IncrementalSFM::findInitialImagePairCandidates(std::vector<std::pair<size_t, size_t>> &initial_pairs, 
                                                    std::vector<size_t> &initial_edge_ids) {
    const base::Scene * const scene = reconstruction_->getScene(); 
    const std::unordered_map<size_t, base::Scene::Edge*> edges = scene->getEdges();
    const std::unordered_map<size_t, base::Scene::Node*> nodes = scene->getNodes();
    ...
    for (const auto &id_edge : edges) {
        if (id_edge.second->model_type != base::Scene::Edge::SceneEdgeModelType::GENERAL_AND_CALIBRATED && 
            id_edge.second->model_type != base::Scene::Edge::SceneEdgeModelType::GENERAL_AND_UNCALIBRATED && 
            id_edge.second->model_type != base::Scene::Edge::SceneEdgeModelType::PANORAMIC && 
            id_edge.second->model_type != base::Scene::Edge::SceneEdgeModelType::PLANNAR && 
            id_edge.second->model_type != base::Scene::Edge::SceneEdgeModelType::PANORAMIC_OR_PLANNAR) {
            continue;
        }

        if (static_cast<int>(id_edge.second->correspondences.size()) < config_.min_num_init_corrs) {
            continue; 
        }

        // if (std::abs(id_edge.second->two_view_geom_info.median_triangulation_angle) < 
        //     util::deg2Rad(std::abs(config_.min_tri_angle_deg_init))) {
        //     continue;
        // }

        if (static_cast<double>(id_edge.second->two_view_geom_info.triangulated_points.size()) / 
            static_cast<double>(id_edge.second->correspondences.size()) < config_.min_init_two_view_geo_tri_rate) {
                continue;
        }

        size_t num_correspondences = id_edge.second->correspondences.size(); 
        for (const size_t &node_id : {id_edge.second->node_ids[0], id_edge.second->node_ids[1]}) {
            const base::Scene::Node *node = scene->getNode(node_id); 
            for (size_t point_2D_idx = 0; point_2D_idx < node->points_2D.size(); ++point_2D_idx) {
                num_correspondences += scene->findDirectCorrespondences(node_id, point_2D_idx).size(); 
            }
        }

        EdgeInfo edge_info;
        edge_info.edge_id = id_edge.first;
        edge_info.num_triangulated_points = id_edge.second->two_view_geom_info.triangulated_points.size();
        edge_info.median_triangulated_angle = id_edge.second->two_view_geom_info.median_triangulation_angle;
        edge_info.num_connected_nodes_1 = scene->getNode(id_edge.second->node_ids[0])->connected_node_ids.size();
        edge_info.num_connected_nodes_2 = scene->getNode(id_edge.second->node_ids[1])->connected_node_ids.size(); 
        edge_info.num_correspondences = num_correspondences; 
        edge_info.model_type = id_edge.second->model_type; 
        // std::cout << "\t" << edge_info.edge_id << " " << id_edge.first << std::endl;

        candidate_infos.push_back(edge_info);
        // std::cout << id_edge.second->two_view_geom_info.median_triangulation_angle << std::endl; 
    ...
    /* Sort the candidates such that edges with more triangulated points, more correspondences, and larger baselines 
       are preferred */
    std::sort(candidate_infos.begin(), candidate_infos.end(), 
        [this](const EdgeInfo &edge_info1, const EdgeInfo &edge_info2) {
            if (edge_info1.model_type == base::Scene::Edge::SceneEdgeModelType::GENERAL_AND_CALIBRATED && 
                edge_info2.model_type != base::Scene::Edge::SceneEdgeModelType::GENERAL_AND_CALIBRATED) {
                return true; 
            } else if (edge_info1.model_type != base::Scene::Edge::SceneEdgeModelType::GENERAL_AND_CALIBRATED && 
                       edge_info2.model_type == base::Scene::Edge::SceneEdgeModelType::GENERAL_AND_CALIBRATED) {
                return false; 
            } else if (edge_info1.model_type == base::Scene::Edge::SceneEdgeModelType::PLANNAR && 
                       edge_info2.model_type == base::Scene::Edge::SceneEdgeModelType::PANORAMIC) {
                return true; 
            } else if (edge_info1.model_type == base::Scene::Edge::SceneEdgeModelType::PANORAMIC && 
                       edge_info2.model_type == base::Scene::Edge::SceneEdgeModelType::PLANNAR) {
                return false; 
            } else if (edge_info1.model_type == base::Scene::Edge::SceneEdgeModelType::GENERAL_AND_UNCALIBRATED && 
                       edge_info2.model_type != base::Scene::Edge::SceneEdgeModelType::GENERAL_AND_UNCALIBRATED) {
                return true; 
            } else if (edge_info1.model_type != base::Scene::Edge::SceneEdgeModelType::GENERAL_AND_UNCALIBRATED && 
                       edge_info2.model_type == base::Scene::Edge::SceneEdgeModelType::GENERAL_AND_UNCALIBRATED) {
                return false; 
    ...
    for (size_t i = 0; i < candidate_infos.size(); ++i) {
        ...
        // std::cout << edge_info.median_triangulated_angle << " " << scene_->getEdge(edge_id)->two_view_geom_info.median_triangulation_angle << std::endl; 
    }

    return true;
}

bool IncrementalSFM::registerInitialImagePair(const std::pair<size_t, size_t> &initial_pair, 
                                              size_t initial_edge_id) {
    std::cout << "--------------Trying to register initial pair (" << initial_pair.first << ", " << initial_pair.second << ")--------------" << std::endl; 
    
    base::Scene *scene = reconstruction_->getScene(); 
    base::Scene::Edge* const initial_edge = scene->getEdge(initial_edge_id);
    assert((initial_edge->node_ids[0] == initial_pair.first && initial_edge->node_ids[1] == initial_pair.second)); 

    /* Mark the nodes as registered */
    if (!reconstruction_->registerInitialImagePair(initial_pair, initial_edge_id)) {
        return false; 
    }
    reconstruction_->clearNewImageIDs(); 
    ...
    /* Recover pose from two-view geometry information. Since the initial pair is always general and 
       calibrated, the relative pose is already available in the two-view geometry information */
    ...
    node2->setPose(Eigen::Quaterniond(initial_edge->two_view_geom_info.R.inverse()), 
                                      -initial_edge->two_view_geom_info.R.inverse() * initial_edge->two_view_geom_info.t); 
    
    /* Compute initial tracks and triangulate them */
    const std::vector<std::pair<size_t, size_t>> &correspondences = initial_edge->correspondences;
    const Eigen::Matrix3x4d proj1 = Eigen::Matrix3x4d::Identity();
    const Eigen::Matrix3x4d proj2 = base::composePose(initial_edge->two_view_geom_info.R, initial_edge->two_view_geom_info.t); 
    ...
    for (auto &corr : correspondences) {
        ...
        const Eigen::Vector3d point_3D = base::triangulatePoint(proj1, proj2, point_2D1N, point_2D2N);
        // const double reproj_error1 = base::computeReprojectionError(point_3D, point_2D1, proj1);
        // const double reproj_error2 = base::computeReprojectionError(point_3D, point_2D2, proj2);
        const double tri_angle_rad = 
            base::computeTriangulationAngle(node1->getTranslation(), 
        ...
        if (base::hasPointPositiveDepth(proj1, point_3D) && 
            ...
            tri_angle_rad >= util::deg2Rad(config_.min_tri_angle_deg_init)) {
                const size_t track_id = reconstruction_->addTrack({base::TrackElement(initial_pair.first, corr.first), 
                                          base::TrackElement(initial_pair.second, corr.second)}, 
                                          point_3D, true);
                reconstruction_->addNewTrackID(track_id); 
                ++num_initial_tracks; 
            // if (reproj_error1 <= config_.max_track_reproj_error && reproj_error2 <= config_.max_track_reproj_error) {
            //     addTrack({base::TrackElement(initial_pair.first, corr.first), 
            //               base::TrackElement(initial_pair.second, corr.second)}, 
            //               point_3D, true);
            // } else {
            //     // std::cout << reproj_error1 << ", " << reproj_error2 << std::endl;
            // }
        } else {
            // std::cout << "[INFO] Add initial track failed. ";
            // std::cout << "point3D: [" << point_3D.transpose() << "] ["
            //           << (initial_edge->two_view_geom_info.R * point_3D + initial_edge->two_view_geom_info.t).transpose() << "] t: [" 
            //           << node2->getTranslation().transpose() 
            //           << "], tri_angle: " << util::rad2Deg(tri_angle_rad) << " deg(s)" << std::endl;
        }
    }
    ...
    std::cout << "median triangulation angle from initial two-view geometry: " 
              << util::rad2Deg(initial_edge->two_view_geom_info.median_triangulation_angle) << " deg(s)" << std::endl; 
    std::cout << "initial_pose1: \n" << 
                 node1->getPose() << std::endl << 
                 "initial_pose2: \n" << 
                 node2->getPose() << std::endl;
    std::cout << "Created " << num_initial_tracks << " initial tracks. " << std::endl;
...
bool IncrementalSFM::findNextImage(size_t &next_image_id, 
...
bool IncrementalSFM::findNextImagesWithMaxVisibleTracks(std::vector<size_t> &next_image_ids, 
    ...
    for (auto &node : nodes) {
        ...
        for (size_t point_2D_idx = 0; point_2D_idx < node.second->points_2D.size(); ++point_2D_idx) {
            const auto &corrs_2D2D = 
                // scene->findTransitiveCorrespondences(image_id, point_2D_idx, config_.max_transitivity); 
            ...
            for (const auto &corr_2D2D : corrs_2D2D) {
                ...
                if (scene->hasNodeBogusCameraParameters(corr_image_id, 
                                                         config_.min_focal_length_ratio, 
        ...
        for (size_t i = 0; i < temp_track_ids.size(); ++i) {
                image_info->corrs_3D2D.insert(std::make_pair(temp_track_ids[i], temp_feature_idxs[i]));
        }
...
bool IncrementalSFM::findNextImageCandidates(std::vector<size_t> &next_image_ids, 
                                 std::vector<std::unordered_map<size_t, size_t>> &correspondences_3D2Ds) {
    bool success = false; 
    ...
    {
    case NextBestViewSelectionMethod::MAX_VISIBLE_TRACK_NUM:
        success = findNextImagesWithMaxVisibleTracks(next_image_ids, correspondences_3D2Ds);
        break;
...
bool IncrementalSFM::registerNextImage(const size_t &image_id, 
    ...
    if (correspondences_3D2D.size() < estimator::AP3PPoseEstimator::kMinNumSamples) {
        return false;
    }
    for (const auto &corr : correspondences_3D2D) {
        if (!existsTrack(corr.first)) {
            std::cout << "[WARNING] track " << corr.first << " does not exits. " << std::endl;
            continue;
        }

        if (corr.second >= node->points_2D.size()) {
            std::cout << "[WARNING] obervation index " << corr.second 
                      << " exceeds the size limit " << node->points_2D.size() << std::endl;
            continue;
        } 
        ...
        points_2D.push_back(node->pix2Norm(node->points_2D[observation_idx].position));
    }
    
    // Solve for the absolute pose using RANSAC P3P
    estimator::RANSACConfig estimator_config;
    estimator_config.max_inlier_error = node->thresholdPix2Norm(config_.max_abs_pose_reproj_error);
    ...
    if (points_3D.size() < estimator.getEstimator().kMinNumSamples ||
        ...
        std::cout << "[ERROR] Insufficient points for robust absolute pose estimation. Registration for image " << image_id << " failed. " << std::endl;
        return false;
    }
    ...
    AbsolutePoseEstimator::Report report = estimator.estimate(points_3D, points_2D);
    if (!report.success) {
        std::cout << "[ERROR] Robust absolute pose estimation failed. Registration for image " << image_id << " failed. " << std::endl;
        return false;
    }

    const double inlier_ratio = 
        static_cast<double>(report.num_inliers) / static_cast<double>(points_3D.size()); 
    if (report.num_inliers < config_.min_abs_pose_inlier_num && 
        inlier_ratio < config_.min_abs_pose_inlier_ratio) {
        std::cout << "[ERROR] Insufficient inlier number of robust absolute pose estimation " 
                  << "(inlier number: " << report.num_inliers << "). "
                  << "Registration for image " << image_id << " failed. " << std::endl;
        return false;
    }

    // const double inlier_ratio = static_cast<double>(report.num_inliers) / static_cast<double>(report.inlier_mask.size()); 
    // if (inlier_ratio < config_.min_abs_pose_inlier_ratio) {
    //     std::cout << "[ERROR] Insufficient inlier ratio of robust absolute pose estimation " 
    ...
    for (size_t i = 0; i < report.inlier_mask.size(); ++i) {
        if (report.inlier_mask[i] && added_track_ids.insert(track_ids[i]).second && 
            ...
            reconstruction_->addObservation(track_ids[i], image_id, observation_idxs[i]);
...
IncrementalSFM::refineAbsolutePose(const size_t &image_id) {
    ...
    ba_config.cam_opt_mode = base::BundleAdjustment::CameraOptimizationMode::ONLY_POSE; 
    ba_config.optimization_algorithm = base::BundleAdjustment::ALGORITHM_LEVENBERG_MARQUART_SPARSE_SHUR; 
    ...
    ba_solver.addCamera(image_id, 
                        node->width, node->height, 
                        false, 
                        node->getRotation(), node->getTranslation(), 
                        points_2D, 
                        base::RadialPinHoleCamera(node->getCameraMatrix(), 
                                                  node->camera_info.k1, node->camera_info.k2));  
    // Collect all 3D-2D  
    for (const auto &obs : node->observations) {
        ...
        ba_solver.addPoint3D(track_id, 
                             track->get3DPosition(), 
                             observations, 
                             true); 
    }

    std::cout << "[INFO] Refining absolute pose for image " << image_id << std::endl; 
    const auto ba_report = ba_solver.optimize(); 
    report.num_iter = ba_report.num_iter; 
    report.final_cost = ba_report.final_cost; 
    report.final_squared_reprojection_error = ba_report.final_squared_reprojection_error; 
    ...
    if (report.success) {
        ...
        node->setPose(ba_camera->rotation, 
                      ba_camera->translation); 
        node->setIntrinsics(ba_camera->camera.fx_, ba_camera->camera.fy_, 
                                ba_camera->camera.cx_, ba_camera->camera.cy_, 
                                ba_camera->camera.alpha_, 
                                ba_camera->camera.k1_, ba_camera->camera.k2_); 
...
IncrementalSFM::solveGlobalBundleAdjustment(const bool &should_update) {
    ...
    const base::Scene * const scene = reconstruction_->getScene(); 
    if (tracks.empty()) {
        std::cout << __PRETTY_FUNCTION__ << " : There is no available track. Global bundle adjustment aborted. " << std::endl;
        return gba_report;
    }
    if (registered_image_ids.empty()) {
        std::cout << __PRETTY_FUNCTION__ << " : There is no available image. Global bundle adjustment aborted. " << std::endl;
        return gba_report;
    }
    ...
    for (auto &image_id : registered_image_ids) {
        ...
        if (config_.gba_config.ba_config.cam_opt_mode == base::BundleAdjustment::CameraOptimizationMode::FULL || 
            config_.gba_config.ba_config.cam_opt_mode == base::BundleAdjustment::CameraOptimizationMode::FULL_AND_SEPARATED) {
        ...
        if (!ba_solver.addCamera(image_id, 
                                 node->width, node->height, 
                                 fix_in, fix_t, fix_r, 
                                 node->getRotation(), node->getTranslation(), 
                                 points_2D, 
            ...
            std::cout << __PRETTY_FUNCTION__ << 
                      " : Adding camera " << image_id << " to BA problem failed. Global bundle adjustment aborted. " << std::endl;
            return gba_report;
        }
    ...
    for (auto &track : tracks) {
        ...
        for (auto &element : elements) {
            if (isImageRegistered(element.image_id) && 
                ...
                observations.insert(std::make_pair(element.image_id, element.feature_idx));
            }
        }
        const size_t kMinFixedTrackLength = 15; 
        const bool fixed = (observations.size() < kMinFixedTrackLength) ? false : true; 
        if (!ba_solver.addPoint3D(track.first, 
                                  track.second->get3DPosition(), 
                                  observations, 
                                  fixed)) {
            std::cout << __PRETTY_FUNCTION__ << 
                          " : Adding track " << track.first << " to BA problem failed. Global bundle adjustment aborted. " << std::endl;
                return gba_report;
        }
    ...
    gba_report.final_squared_reprojection_error = ba_report.final_squared_reprojection_error; 
    ...
    if (!ba_report.success) {
        std::cout << __PRETTY_FUNCTION__ << ": Optimization failed. Global bundle adjustment aborted. " << std::endl; 
    }
    // Update parameters of cameras and coordinates of tracks according to the result of BA 
    // Filter images with bogus camera parameters 
    if (ba_report.success) {
        updateStructureAndMotionFromBA(ba_solver);
    }
...
IncrementalSFM::solveLocalBundleAdjustment(const size_t &image_id, 
    ...
    const base::Scene * const scene = reconstruction_->getScene(); 
    if (tracks.empty()) {
        std::cout << __PRETTY_FUNCTION__ << " : There is no available track. Local bundle adjustment aborted. " << std::endl;
        return lba_report;
    }
    if (registered_image_ids.empty()) {
        std::cout << __PRETTY_FUNCTION__ << " : There is no available image. Local bundle adjustment aborted. " << std::endl;
        return lba_report;
    } 
    if (!existsImage(image_id)) {
        std::cout << __PRETTY_FUNCTION__ 
            << " : Image " << image_id << " does not exist. Local bundle adjustment aborted. " << std::endl;
        return lba_report;
    }
    ...
    for (const auto &obs : node->observations) {
        ...
        for (const auto &elem : track->getElements()) {
            ...
            if (!existsImage(elem.image_id) || 
                isImageFiltered(elem.image_id)) {
                continue; 
            }
    ...
    const size_t num_related_images = std::min(kMaxNumRelatedImages, nums_covisible_vec.size()); 
    ...
    for (const size_t &camera_id : camera_ids) {
        ...
        EigenVec<Eigen::Vector2d> points_2D; 
        for (size_t point_2D_idx = 0; point_2D_idx < n->points_2D.size(); ++point_2D_idx) {
            points_2D.push_back(n->points_2D[point_2D_idx].position); 
        }
        bool fix_in{false}, fix_t{false}, fix_r{false}; 
        if (config_.gba_config.ba_config.cam_opt_mode == base::BundleAdjustment::CameraOptimizationMode::FULL || 
            config_.gba_config.ba_config.cam_opt_mode == base::BundleAdjustment::CameraOptimizationMode::FULL_AND_SEPARATED) {
            if (camera_id == reconstruction_->getRefImageID0() || camera_id == local_ref_image_id_0) {
                fix_t = true; 
                fix_r = true; 
            } else if (image_id == reconstruction_->getRefImageID1() || local_ref_image_id_1) {
                fix_t = true; 
            }
        } else {
            if (camera_id == reconstruction_->getRefImageID0() || local_ref_image_id_0) {
                fix_in = true; 
                fix_t = true; 
                fix_r = true; 
            }
        }
        if (!ba_solver.addCamera(camera_id, 
                                 n->width, n->height, 
                                 fix_in, fix_t, fix_r, 
                                 n->getRotation(), n->getTranslation(), 
                                 points_2D, 
                                 base::RadialPinHoleCamera(n->getCameraMatrix(), 
                                                            n->camera_info.k1, n->camera_info.k2))) {
            std::cout << __PRETTY_FUNCTION__ << 
                      " : Adding camera " << image_id << " to BA problem failed. Global bundle adjustment aborted. " << std::endl;
            return lba_report;
        } 
    ...
    for (const size_t &track_id : track_ids) {
        ...
        if (!ba_solver.addPoint3D(track_id, 
                                  track->get3DPosition(), 
                                  observations)) {
            std::cout << __PRETTY_FUNCTION__ << 
                          " : Adding track " << track_id << " to BA problem failed. Global bundle adjustment aborted. " << std::endl;
                return lba_report;
        }
    ...
    lba_report.final_squared_reprojection_error = ba_report.final_squared_reprojection_error; 
    ...
    if (should_update) {
        ...
        /* Complete tracks */
        lba_report.num_completed_observations += completeTracks(lba_report.track_ids); 
    }
...
void IncrementalSFM::updateStructureAndMotionFromBA(const base::BundleAdjustment &ba_solver) {
    ...
    for (auto &ba_camera : ba_cameras) {
        ...
        if (!isImageRegistered(camera_id) || 
            isImageFiltered(camera_id)) {
            continue;
        }
        ...
        if (ba_camera.second->hasBogusParams(config_.min_focal_length_ratio, 
            ...
            std::cout << "[INFO] Removed camera " << camera_id << " due to bogus parameters. " << std::endl;  
            continue;
        }

        base::Scene::Node* const node = reconstruction_->getScene()->getNode(camera_id); 
        if (node != nullptr) {
            node->setPose(ba_camera.second->rotation, ba_camera.second->translation); 
            node->setIntrinsics(ba_camera.second->camera.fx_, ba_camera.second->camera.fy_, 
                                ba_camera.second->camera.cx_, ba_camera.second->camera.cy_, 
                                ba_camera.second->camera.alpha_, 
                                ba_camera.second->camera.k1_, ba_camera.second->camera.k2_); 
        }
    }
...
size_t IncrementalSFM::mergeTrack(const size_t &track_id) {
    ...
    for (auto &element : track->getElements()) {
        if (!existsImage(element.image_id)) {
            reconstruction_->removeObservation(track_id, element.image_id, element.feature_idx);
            continue; 
        }
        ...
        std::vector<std::pair<size_t, size_t>> correspondences = 
            ...
            reconstruction_->getScene()->findTransitiveCorrespondences(image_id, point_2D_idx, config_.max_transitivity); 
        for (auto &corr : correspondences) {
            size_t corr_track_id; 
            if (!(reconstruction_->getScene()->hasTrack(corr.first, corr.second, &corr_track_id))) {
                continue; 
            }
            ...
            const Eigen::Vector3d merge_pos = (len_track * track->get3DPosition() + 
            ...
            for (auto &elems : {track->getElements(), corr_track->getElements()}) {
                for (auto &elem : elems) {
                    if (reconstruction_->getScene()->computeReprojectionError(elem.image_id, elem.feature_idx, merge_pos) > 
            ...
            if (success) {
                ...
                // Add new track 
                const size_t merge_track_id = reconstruction_->addTrack(merge_elements, merge_pos, true); 
...
size_t IncrementalSFM::mergeTracks(const std::vector<size_t> &track_ids) {
    ...
    for (size_t i = 0; i < track_ids.size(); ++i) {
        ...
        std::cout << "\r[INFO] Merging tracks... " << i + 1 << "/" << track_ids.size(); 
    }
    std::cout << std::endl; 
...
size_t IncrementalSFM::triangulateImage(const size_t &image_id) {
    ...
    base::Scene::Node* const node = reconstruction_->getScene()->getNode(image_id); 
    ...
    for (size_t point_2D_idx = 0; point_2D_idx < node->points_2D.size(); 
        ...
        if (node->hasTrack(point_2D_idx, &existing_track_id)) {
            if (existsTrack(existing_track_id)) {
                base::Track *existing_track = reconstruction_->getTrack(existing_track_id); 
                // If a track already exists but has not been triangulated, triangulate it
        ...
        const std::vector<std::pair<size_t, size_t>> correspondences = 
            // scene_->findDirectCorrespondences(image_id, point_2D_idx);
            scene->findTransitiveCorrespondences(image_id, point_2D_idx, config_.max_transitivity); 
        // std::cout << "[INFO] Found " << correspondences.size() << " correspondences for feature (index = " 
        //           << point_2D_idx << ") in image (id = " << image_id << ")" << std::endl;
        ...
        if (track_elems.size() < 
            std::max(std::max(config_.min_tri_num_obs, 2UL), config_.min_track_length)) {
            continue;
        } else {
            const size_t track_id = reconstruction_->addTrack(track_elems, Eigen::Vector3d::Zero(), false); 
            // std::cout << "[INFO] Trying to triangulate track " << track_id << std::endl; 
...
size_t IncrementalSFM::triangulateTrack(const size_t &track_id) {
    ...
    tri_config.max_inlier_error = util::deg2Rad(config_.max_tri_inlier_angular_error_deg); 
    ...
    if (!report.success) {
        ...
        reconstruction_->removeTrack(track_id); 
        // std::cout << "[ERROR] Robust triangulation failed for track " << track_id << std::endl;
        return num_tris; 
    }
    ...
    double inlier_ratio = 
        static_cast<double>(inliers.size()) / static_cast<double>(elements.size()); 
    if (exists_duplicated_images || 
        inlier_ratio < config_.min_tri_inlier_ratio || 
        inliers.size() < config_.min_track_length) {
        reconstruction_->removeTrack(track_id); 
        // std::cout << "[INFO] track " << track_id << " deleted due to too few inliers. " << std::endl;
        ...
        // std::cout << "[INFO] Filtered " << prev_len - inliers.size() << " outliers in track " << track_id << std::endl;
    }
    reconstruction_->addNewTrackID(track_id); 
        ...
        while (elements.size() >= 3UL)
        {
            ...
            if (!report.success) {
                ...
                if (!exists_duplicated_images &&  
                    ...
                    //           << " from " << elements.size() << " outliers of track " << track_id << std::endl; 
                    num_tris += inliers.size();  
                } else {
                    break;
                }
...
IncrementalSFM::triangulateObservations(const typename estimator::RANSACConfig &tri_config, 
    ...
    for (size_t elem_idx = 0; elem_idx < elements.size(); ++elem_idx) {
        ...
        const Eigen::Vector2d point_2D = 
            scene->getNodePoint2DPosition(element.image_id, 
                                           element.feature_idx); 
        const Eigen::Matrix3d rotation = base::quaternion2RotationMatrix(scene->getNodeRotation(element.image_id)); 
        const Eigen::Vector3d translation = scene->getNodeTranslation(element.image_id); 
        const Eigen::Matrix3x4d proj = base::composePose(rotation.inverse(), -rotation.inverse() * translation); 
        const base::CameraBase *camera = new base::RadialPinHoleCamera(scene->getNodeCameraMatrix(element.image_id)); 
    ...
    if (point_data.size() < 2) {
        std::cout << "[ERROR] [Observation Triangulation] View number is less than 2. Cannot triangulate. " << std::endl;
        report.success = false; 
        return report; 
    }
    ...
    estimator.getEstimator().setEstimationErrorType(estimator::TriangulationEstimator::EstimationErrorType::ANGULAR_ERROR); 
    estimator.getLocalEstimator().setMinTriAngleDeg(config_.min_tri_angle_deg); 
    estimator.getLocalEstimator().setEstimationErrorType(estimator::TriangulationEstimator::EstimationErrorType::ANGULAR_ERROR); 
...
size_t IncrementalSFM::completeTrack(const size_t &track_id) {
    ...
    base::Scene * const scene = reconstruction_->getScene(); 
    base::TrackElementList queue = reconstruction_->getTrack(track_id)->getElements();
    std::unordered_set<size_t> added_image_ids; 
    ...
    for (size_t transitivity = 0; transitivity < config_.max_transitivity; ++transitivity) {
        if (queue.empty()) {
            break;
        }
        ...
        for (const auto &element : prev_queue) {
            ...
            const std::vector<std::pair<size_t, size_t>> corrs = 
                scene->findDirectCorrespondences(element.image_id, element.feature_idx); 
            for (const auto &corr : corrs) {
                ...
                const base::Scene::Node* const corr_node = scene->getNode(corr_image_id); 
                if (corr_node == nullptr) {
                    continue;
                }
                // Check reprojection error
                const double reproj_error = 
                    corr_node->computeReprojectionError(reconstruction_->getTrack(track_id)->get3DPosition(), 
                                                        corr_feature_idx); 
                if (reproj_error <= config_.max_track_reproj_error) {
                    if (added_image_ids.insert(corr_image_id).second && 
                        !reconstruction_->hasTrack(corr_image_id, corr_feature_idx, nullptr)) {
                        reconstruction_->addObservation(track_id, corr_image_id, corr_feature_idx); 
...
size_t IncrementalSFM::completeTracks(const std::vector<size_t> &track_ids) {
    ...
    for (size_t i = 0; i < track_ids.size(); ++i) {
        ...
        std::cout << "\r[INFO] Completing tracks... " << i + 1 << "/" << track_ids.size(); 
    }
    std::cout << std::endl; 
...
size_t IncrementalSFM::filterTracks(const std::vector<size_t> &track_ids, 
    ...
    num_filtered += filterTracksWithLargeReprojectionError(config_.max_track_reproj_error, 
                                                           track_ids, 
                                                           init); 
    std::cout << "[INFO] Filtered " << num_filtered << " observations with large reprojection error. " << std::endl; 
    size_t num_filtered_small_tri_ang = filterTracksWithSmallTriangulationAngle(util::deg2Rad(config_.min_tri_angle_deg), 
                                                            track_ids, 
                                                            init); 
    std::cout << "[INFO] Filtered " << num_filtered_small_tri_ang << " observations with small triangulation angles. " << std::endl; 
    num_filtered += num_filtered_small_tri_ang; 
    return num_filtered; 
}

size_t IncrementalSFM::filterTracksWithLargeReprojectionError(const double &max_reproj_error, 
    ...
    base::Scene * const scene = reconstruction_->getScene(); 
    for (const size_t &track_id : track_ids) {
        // std::cout << "\r[INFO] Filtering tracks... " << num_checked_tracks++ << "/" << track_ids.size();
        if (!existsTrack(track_id)) {
            continue; 
        }
        base::Track* const track = reconstruction_->getTrack(track_id);
        ...
        if (!init && track->length() < config_.min_track_length) {
            // std::cout << "[WARNING] Removed track " << track_id << " due to insufficient length. " << std::endl;
        ...
        for (const auto &element : track->getElements()) {
            ...
            const double reproj_error = 
                scene->computeReprojectionError(element.image_id, element.feature_idx, position); 
        ...
        if (!init && track->length() < config_.min_track_length) {
            // std::cout << "[WARNING] Removed track " << track_id << " due to large reprojection errors. " << std::endl;
            num_filtered += track->length(); 
            reconstruction_->removeTrack(track_id); 
...
size_t IncrementalSFM::filterTracksWithSmallTriangulationAngle(const double &min_tri_angle_rad, 
                                                               const std::vector<size_t> &track_ids, 
                                                               const bool &init) {
    size_t num_filtered = 0; 
    // size_t num_checked_tracks = 0; 
    ...
    for (const size_t &track_id : track_ids) {
        ...
        const Eigen::Vector3d &position = track->get3DPosition();  
        // Filter tracks with no pair of observations that has sufficient triangulation angle
        ...
        for (int64_t elem_idx1 = 0; elem_idx1 < static_cast<int64_t>(elements.size()) - 1; ++elem_idx1) {
            ...
            for (int64_t elem_idx2 = elem_idx1 + 1; elem_idx2 < static_cast<int64_t>(elements.size()); ++elem_idx2) {
                ...
                const double tri_angle_rad = base::computeTriangulationAngle(t1, t2, position); 
        ...
        if (!should_keep) {
            // std::cout << "[WARNING] Removed track " << track_id << " due to small triangulation angles. " << std::endl;
            num_filtered += track->length(); 
            reconstruction_->removeTrack(track_id); 
        } else {
            ...
            for (const size_t &nonex_elem_idx : nonex_elem_idxs) {
                ...
                reconstruction_->removeObservation(track_id, nonex_elem.image_id, nonex_elem.feature_idx); 
                num_filtered++; 
            }
        }
    }
